...
def test_PlanDelta_get_dimens_cruds_plandelta_ReturnsObjWithCorrectDimensAndCRUDsBy_partnerunit_insert():
    # ESTABLISH
    before_sue_plan = planunit_shop(exx.sue)
    before_sue_plan.add_partnerunit(exx.yao)
    ...
    bob_planatom = sue_partnerunit_dict.get(exx.bob)
    assert bob_planatom.get_value(kw.partner_name) == exx.bob
    assert bob_planatom.get_value("partner_cred_lumen") == bob_partner_cred_lumen
    assert bob_planatom.get_value("partner_debt_lumen") == bob_partner_debt_lumen
