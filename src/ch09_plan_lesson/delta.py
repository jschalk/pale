...
from src.ch08_plan_atom.atom_config import CRUD_command
...
from src.ch09_plan_lesson._ref.ch09_semantic_types import RopeTerm, TitleTerm
...
class PlanDelta:
    ...
    def get_atom_edited_plan(self, before_plan: PlanUnit) -> PlanUnit:
    ...
    def c_planatom_exists(self, x_planatom: PlanAtom) -> bool:
        if x_planatom.is_valid() is False:
            raise InvalidPlanAtomException(
                f"""'{x_planatom.dimen}' {x_planatom.crud_str} PlanAtom is invalid
                {x_planatom.is_jkeys_valid()=}
    ...
    def add_planatoms_planunit_simple_attrs(
        ...
        if before_plan.fund_grain != after_plan.fund_grain:
            x_planatom.set_jvalue("fund_grain", after_plan.fund_grain)
        if before_plan.respect_grain != after_plan.respect_grain:
    ...
    def add_planatom_membership_update(
        ...
        if after_membership.group_debt_lumen != before_membership.group_debt_lumen:
            x_planatom.set_jvalue("group_debt_lumen", after_membership.group_debt_lumen)
        self.set_planatom(x_planatom)
    ...
    def add_planatoms_kegs(self, before_plan: PlanUnit, after_plan: PlanUnit):
        before_keg_ropes = set(before_plan._keg_dict.keys())
        after_keg_ropes = set(after_plan._keg_dict.keys())

        self.add_planatom_keg_inserts(
            after_plan=after_plan,
            insert_keg_ropes=after_keg_ropes.difference(before_keg_ropes),
        )
        self.add_planatom_keg_deletes(
            before_plan=before_plan,
            delete_keg_ropes=before_keg_ropes.difference(after_keg_ropes),
        )
        self.add_planatom_keg_updates(
            before_plan=before_plan,
            after_plan=after_plan,
            update_ropes=before_keg_ropes & (after_keg_ropes),
        )

    def add_planatom_keg_inserts(self, after_plan: PlanUnit, insert_keg_ropes: set):
        for insert_keg_rope in insert_keg_ropes:
            insert_kegunit = after_plan.get_keg_obj(insert_keg_rope)
            x_planatom = planatom_shop("plan_kegunit", "INSERT")
            x_planatom.set_jkey("keg_rope", insert_kegunit.get_keg_rope())
            x_planatom.set_jvalue("addin", insert_kegunit.addin)
            x_planatom.set_jvalue("begin", insert_kegunit.begin)
            x_planatom.set_jvalue("close", insert_kegunit.close)
            x_planatom.set_jvalue("denom", insert_kegunit.denom)
            x_planatom.set_jvalue("numor", insert_kegunit.numor)
            x_planatom.set_jvalue("morph", insert_kegunit.morph)
            x_planatom.set_jvalue("star", insert_kegunit.star)
            x_planatom.set_jvalue("pledge", insert_kegunit.pledge)
            self.set_planatom(x_planatom)

            self.add_planatom_keg_factunit_inserts(
                kegunit=insert_kegunit,
                insert_factunit_reason_contexts=set(insert_kegunit.factunits.keys()),
            )
            self.add_planatom_keg_awardunit_inserts(
                after_kegunit=insert_kegunit,
                insert_awardunit_awardee_titles=set(insert_kegunit.awardunits.keys()),
            )
            self.add_planatom_keg_reasonunit_inserts(
                after_kegunit=insert_kegunit,
                insert_reasonunit_reason_contexts=set(
                    insert_kegunit.reasonunits.keys()
                ),
            )
            self.add_planatom_keg_partyunit_insert(
    ...
    def add_planatom_keg_updates(
        ...
        for keg_rope in update_ropes:
            after_kegunit = after_plan.get_keg_obj(keg_rope)
            before_kegunit = before_plan.get_keg_obj(keg_rope)
            if jvalues_different("plan_kegunit", before_kegunit, after_kegunit):
                x_planatom = planatom_shop("plan_kegunit", "UPDATE")
                x_planatom.set_jkey("keg_rope", after_kegunit.get_keg_rope())
                if before_kegunit.addin != after_kegunit.addin:
                    x_planatom.set_jvalue("addin", after_kegunit.addin)
                if before_kegunit.begin != after_kegunit.begin:
                    x_planatom.set_jvalue("begin", after_kegunit.begin)
                if before_kegunit.close != after_kegunit.close:
                    x_planatom.set_jvalue("close", after_kegunit.close)
                if before_kegunit.denom != after_kegunit.denom:
                    x_planatom.set_jvalue("denom", after_kegunit.denom)
                if before_kegunit.numor != after_kegunit.numor:
                    x_planatom.set_jvalue("numor", after_kegunit.numor)
                if before_kegunit.morph != after_kegunit.morph:
                    x_planatom.set_jvalue("morph", after_kegunit.morph)
                if before_kegunit.star != after_kegunit.star:
                    x_planatom.set_jvalue("star", after_kegunit.star)
                if before_kegunit.pledge != after_kegunit.pledge:
                    x_planatom.set_jvalue("pledge", after_kegunit.pledge)
                self.set_planatom(x_planatom)
            ...
            self.add_planatom_keg_factunit_updates(
            ...
            self.add_planatom_keg_factunit_deletes(
                keg_rope=keg_rope,
            ...
            self.add_planatom_keg_partyunit_deletes(
                keg_rope=keg_rope,
    ...
    def add_planatom_keg_deletes(self, before_plan: PlanUnit, delete_keg_ropes: set):
        for delete_keg_rope in delete_keg_ropes:
            x_planatom = planatom_shop("plan_kegunit", "DELETE")
            x_planatom.set_jkey("keg_rope", delete_keg_rope)
            self.set_planatom(x_planatom)

            delete_kegunit = before_plan.get_keg_obj(delete_keg_rope)
            self.add_planatom_keg_factunit_deletes(
    ...
    def add_planatom_keg_reasonunit_inserts(
        ...
        for insert_reasonunit_reason_context in insert_reasonunit_reason_contexts:
            ...
            x_planatom = planatom_shop("plan_keg_reasonunit", "INSERT")
            x_planatom.set_jkey("keg_rope", after_kegunit.get_keg_rope())
            x_planatom.set_jkey("reason_context", after_reasonunit.reason_context)
            ...
            self.add_planatom_keg_reason_caseunit_inserts(
    ...
    def add_planatom_keg_reasonunit_updates(
        self,
        ...
        for update_reasonunit_reason_context in update_reasonunit_reason_contexts:
            ...
            if jvalues_different(
                "plan_keg_reasonunit", before_reasonunit, after_reasonunit
            ):
                x_planatom = planatom_shop("plan_keg_reasonunit", "UPDATE")
                x_planatom.set_jkey("keg_rope", before_kegunit.get_keg_rope())
                x_planatom.set_jkey("reason_context", after_reasonunit.reason_context)
                if (
            ...
            self.add_planatom_keg_reason_caseunit_updates(
                keg_rope=before_kegunit.get_keg_rope(),
            ...
            self.add_planatom_keg_reason_caseunit_deletes(
                keg_rope=before_kegunit.get_keg_rope(),
    ...
    def add_planatom_keg_reasonunit_deletes(
        ...
        for delete_reasonunit_reason_context in delete_reasonunit_reason_contexts:
            x_planatom = planatom_shop("plan_keg_reasonunit", "DELETE")
            x_planatom.set_jkey("keg_rope", before_kegunit.get_keg_rope())
            x_planatom.set_jkey("reason_context", delete_reasonunit_reason_context)
            self.set_planatom(x_planatom)
    ...
    def add_planatom_keg_reason_caseunit_inserts(
        self,
        keg_rope: RopeTerm,
        ...
        for insert_case_reason_state in insert_case_reason_states:
            ...
            x_planatom = planatom_shop("plan_keg_reason_caseunit", "INSERT")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("reason_context", after_reasonunit.reason_context)
            x_planatom.set_jkey("reason_state", after_caseunit.reason_state)
            if after_caseunit.reason_lower is not None:
                x_planatom.set_jvalue("reason_lower", after_caseunit.reason_lower)
            if after_caseunit.reason_upper is not None:
                x_planatom.set_jvalue("reason_upper", after_caseunit.reason_upper)
            if after_caseunit.reason_divisor is not None:
                x_planatom.set_jvalue("reason_divisor", after_caseunit.reason_divisor)
            self.set_planatom(x_planatom)

    def add_planatom_keg_reason_caseunit_updates(
        self,
        keg_rope: RopeTerm,
        ...
        for update_case_reason_state in update_case_reason_states:
            ...
            if jvalues_different(
                ...
                x_planatom = planatom_shop("plan_keg_reason_caseunit", "UPDATE")
                x_planatom.set_jkey("keg_rope", keg_rope)
                x_planatom.set_jkey("reason_context", before_reasonunit.reason_context)
                x_planatom.set_jkey("reason_state", after_caseunit.reason_state)
    ...
    def add_planatom_keg_reason_caseunit_deletes(
        self,
        keg_rope: RopeTerm,
        ...
        for delete_case_reason_state in delete_case_reason_states:
            x_planatom = planatom_shop("plan_keg_reason_caseunit", "DELETE")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("reason_context", reasonunit_reason_context)
            x_planatom.set_jkey("reason_state", delete_case_reason_state)
            self.set_planatom(x_planatom)

    def add_planatom_keg_partyunit_insert(
        ...
        for insert_partyunit_party_title in insert_partyunit_party_titles:
            x_planatom = planatom_shop("plan_keg_partyunit", "INSERT")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("party_title", insert_partyunit_party_title)
            self.set_planatom(x_planatom)

    def add_planatom_keg_partyunit_deletes(
        ...
        for delete_partyunit_party_title in delete_partyunit_party_titles:
            x_planatom = planatom_shop("plan_keg_partyunit", "DELETE")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("party_title", delete_partyunit_party_title)
            self.set_planatom(x_planatom)

    def add_planatom_keg_healerunit_insert(
        ...
        for insert_healerunit_healer_name in insert_healerunit_healer_names:
            x_planatom = planatom_shop("plan_keg_healerunit", "INSERT")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("healer_name", insert_healerunit_healer_name)
            self.set_planatom(x_planatom)

    def add_planatom_keg_healerunit_deletes(
        ...
        for delete_healerunit_healer_name in delete_healerunit_healer_names:
            x_planatom = planatom_shop("plan_keg_healerunit", "DELETE")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("healer_name", delete_healerunit_healer_name)
            self.set_planatom(x_planatom)

    def add_planatom_keg_awardunit_inserts(
        ...
        for after_awardunit_awardee_title in insert_awardunit_awardee_titles:
            ...
            x_planatom = planatom_shop("plan_keg_awardunit", "INSERT")
            x_planatom.set_jkey("keg_rope", after_kegunit.get_keg_rope())
            x_planatom.set_jkey("awardee_title", after_awardunit.awardee_title)
            x_planatom.set_jvalue("give_force", after_awardunit.give_force)
            x_planatom.set_jvalue("take_force", after_awardunit.take_force)
            self.set_planatom(x_planatom)

    def add_planatom_keg_awardunit_updates(
        self,
        before_kegunit: KegUnit,
        after_kegunit: KegUnit,
        ...
        for update_awardunit_awardee_title in update_awardunit_awardee_titles:
            ...
            if jvalues_different(
                "plan_keg_awardunit", before_awardunit, after_awardunit
            ):
                x_planatom = planatom_shop("plan_keg_awardunit", "UPDATE")
                x_planatom.set_jkey("keg_rope", before_kegunit.get_keg_rope())
                x_planatom.set_jkey("awardee_title", after_awardunit.awardee_title)
    ...
    def add_planatom_keg_awardunit_deletes(
        ...
        for delete_awardunit_awardee_title in delete_awardunit_awardee_titles:
            x_planatom = planatom_shop("plan_keg_awardunit", "DELETE")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("awardee_title", delete_awardunit_awardee_title)
            self.set_planatom(x_planatom)

    def add_planatom_keg_factunit_inserts(
        ...
        for insert_factunit_reason_context in insert_factunit_reason_contexts:
            ...
            x_planatom = planatom_shop("plan_keg_factunit", "INSERT")
            x_planatom.set_jkey("keg_rope", kegunit.get_keg_rope())
            x_planatom.set_jkey("fact_context", insert_factunit.fact_context)
            if insert_factunit.fact_state is not None:
                x_planatom.set_jvalue("fact_state", insert_factunit.fact_state)
            if insert_factunit.fact_lower is not None:
                x_planatom.set_jvalue("fact_lower", insert_factunit.fact_lower)
            if insert_factunit.fact_upper is not None:
                x_planatom.set_jvalue("fact_upper", insert_factunit.fact_upper)
            self.set_planatom(x_planatom)

    def add_planatom_keg_factunit_updates(
        self,
        before_kegunit: KegUnit,
        after_kegunit: KegUnit,
        ...
        for update_factunit_reason_context in update_factunit_reason_contexts:
            ...
            if jvalues_different("plan_keg_factunit", before_factunit, after_factunit):
                x_planatom = planatom_shop("plan_keg_factunit", "UPDATE")
                x_planatom.set_jkey("keg_rope", before_kegunit.get_keg_rope())
                x_planatom.set_jkey("fact_context", after_factunit.fact_context)
                if before_factunit.fact_state != after_factunit.fact_state:
                    x_planatom.set_jvalue("fact_state", after_factunit.fact_state)
                if before_factunit.fact_lower != after_factunit.fact_lower:
                    x_planatom.set_jvalue("fact_lower", after_factunit.fact_lower)
                if before_factunit.fact_upper != after_factunit.fact_upper:
                    x_planatom.set_jvalue("fact_upper", after_factunit.fact_upper)
                self.set_planatom(x_planatom)

    def add_planatom_keg_factunit_deletes(
        ...
        for delete_factunit_reason_context in delete_factunit_reason_contexts:
            x_planatom = planatom_shop("plan_keg_factunit", "DELETE")
            x_planatom.set_jkey("keg_rope", keg_rope)
            x_planatom.set_jkey("fact_context", delete_factunit_reason_context)
            self.set_planatom(x_planatom)
...
def plan_built_from_delta_is_valid(x_delta: PlanDelta, x_plan: PlanUnit = None) -> bool:
    ...
    x_plan = x_delta.get_atom_edited_plan(x_plan)
    try:
        x_plan.cashout()
    except Exception:
        return False
