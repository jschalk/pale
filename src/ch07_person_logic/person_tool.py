...
def person_kegunit_exists(x_person: PersonUnit, jkeys: dict[str, any]) -> bool:
    x_rope = jkeys.get("keg_rope")
    return False if x_person is None else bool(x_person.keg_exists(x_rope))


def person_keg_awardunit_exists(x_person: PersonUnit, jkeys: dict[str, any]) -> bool:
    x_awardee_title = jkeys.get("awardee_title")
    x_rope = jkeys.get("keg_rope")
    return bool(
        person_kegunit_exists(x_person, jkeys)
        and x_person.get_keg_obj(x_rope).awardunit_exists(x_awardee_title)
    )


def person_keg_reasonunit_exists(x_person: PersonUnit, jkeys: dict[str, any]) -> bool:
    x_rope = jkeys.get("keg_rope")
    x_reason_context = jkeys.get("reason_context")
    return bool(
        person_kegunit_exists(x_person, jkeys)
        and x_person.get_keg_obj(x_rope).reasonunit_exists(x_reason_context)
    )


def person_keg_reason_caseunit_exists(
    x_person: PersonUnit, jkeys: dict[str, any]
) -> bool:
    x_rope = jkeys.get("keg_rope")
    x_reason_context = jkeys.get("reason_context")
    ...
    return bool(
        person_keg_reasonunit_exists(x_person, jkeys)
        and x_person.get_keg_obj(x_rope)
        .get_reasonunit(x_reason_context)
        .case_exists(x_reason_state)
    )


def person_keg_partyunit_exists(x_person: PersonUnit, jkeys: dict[str, any]) -> bool:
    x_party_title = jkeys.get("party_title")
    x_rope = jkeys.get("keg_rope")
    return bool(
        person_kegunit_exists(x_person, jkeys)
        and x_person.get_keg_obj(x_rope).laborunit.partyunit_exists(x_party_title)
    )


def person_keg_healerunit_exists(x_person: PersonUnit, jkeys: dict[str, any]) -> bool:
    x_healer_name = jkeys.get("healer_name")
    x_rope = jkeys.get("keg_rope")
    return bool(
        person_kegunit_exists(x_person, jkeys)
        and x_person.get_keg_obj(x_rope).healerunit.healer_name_exists(x_healer_name)
    )


def person_keg_factunit_exists(x_person: PersonUnit, jkeys: dict[str, any]) -> bool:
    x_rope = jkeys.get("keg_rope")
    x_fact_context = jkeys.get("fact_context")
    return bool(
        person_kegunit_exists(x_person, jkeys)
        and x_person.get_keg_obj(x_rope).factunit_exists(x_fact_context)
    )
...
def person_attr_exists(
    ...
    elif x_dimen == "person_keg_awardunit":
        return person_keg_awardunit_exists(x_person, jkeys)
    elif x_dimen == "person_keg_factunit":
        return person_keg_factunit_exists(x_person, jkeys)
    elif x_dimen == "person_keg_healerunit":
        return person_keg_healerunit_exists(x_person, jkeys)
    elif x_dimen == "person_keg_reason_caseunit":
        return person_keg_reason_caseunit_exists(x_person, jkeys)
    elif x_dimen == "person_keg_reasonunit":
        return person_keg_reasonunit_exists(x_person, jkeys)
    elif x_dimen == "person_keg_partyunit":
        return person_keg_partyunit_exists(x_person, jkeys)
    elif x_dimen == "person_kegunit":
        return person_kegunit_exists(x_person, jkeys)
...
def person_kegunit_get_obj(x_person: PersonUnit, jkeys: dict[str, any]) -> KegUnit:
    x_rope = jkeys.get("keg_rope")
    return x_person.get_keg_obj(x_rope)


def person_keg_awardunit_get_obj(
    x_person: PersonUnit, jkeys: dict[str, any]
) -> AwardUnit:
    x_rope = jkeys.get("keg_rope")
    x_awardee_title = jkeys.get("awardee_title")
    return x_person.get_keg_obj(x_rope).get_awardunit(x_awardee_title)


def person_keg_reasonunit_get_obj(
    x_person: PersonUnit, jkeys: dict[str, any]
) -> ReasonUnit:
    x_rope = jkeys.get("keg_rope")
    x_reason_context = jkeys.get("reason_context")
    return x_person.get_keg_obj(x_rope).get_reasonunit(x_reason_context)


def person_keg_reason_caseunit_get_obj(
    x_person: PersonUnit, jkeys: dict[str, any]
) -> CaseUnit:
    ...
    x_reason_state = jkeys.get("reason_state")
    return (
        x_person.get_keg_obj(x_rope)
        .get_reasonunit(x_reason_context)
        .get_case(x_reason_state)
    )


def person_keg_factunit_get_obj(
    x_person: PersonUnit, jkeys: dict[str, any]
) -> FactUnit:
    x_rope = jkeys.get("keg_rope")
    x_fact_context = jkeys.get("fact_context")
    return x_person.get_keg_obj(x_rope).factunits.get(x_fact_context)
...
def person_get_obj(x_dimen: str, x_person: PersonUnit, jkeys: dict[str, any]) -> any:
    ...
    x_dimens = {
        ...
        "person_kegunit": person_kegunit_get_obj,
        "person_keg_awardunit": person_keg_awardunit_get_obj,
        "person_keg_reasonunit": person_keg_reasonunit_get_obj,
        "person_keg_reason_caseunit": person_keg_reason_caseunit_get_obj,
        "person_keg_factunit": person_keg_factunit_get_obj,
    }
...
def get_person_partner_agenda_award_array(
    x_person: PersonUnit, cashout: bool = None
) -> list[list]:
    if cashout:
        x_person.cashout()
...
def get_person_root_facts_dict(
    ...
    return x_person.get_kegroot_factunits_dict()


def set_factunits_to_person(x_person: PersonUnit, x_facts_dict: dict[RopeTerm, dict]):
    """Sets dict of FactUnits to PersonUnit kegroot"""
    ...
    not_missing_fact_reason_contexts = set(x_person.get_kegroot_factunits_dict().keys())
...
def clear_factunits_from_person(x_person: PersonUnit):
    """Deletes all PersonUnit kegroot FactUnits"""
    for fact_reason_context in get_person_root_facts_dict(x_person).keys():
        x_person.del_fact(fact_reason_context)


def person_keg_reason_caseunit_set_obj(person: PersonUnit, args: dict[str,]):
    """Wrapper for method that edit personunit keg nodes reasonunits.
...
def get_person_unique_short_ropes(person: PersonUnit) -> dict[RopeTerm, RopeTerm]:
    ...
    return get_unique_short_ropes(set(person.get_keg_dict().keys()), person.knot)
...
def add_frame_to_personunit(
    ...
    for x_keg in x_person.get_keg_dict().values():
        for x_reason in x_keg.reasonunits.values():
            if is_sub_rope(x_reason.reason_context, required_context_subrope):
                reason_context_keg = x_person.get_keg_obj(x_reason.reason_context)
        ...
        for x_fact in x_keg.factunits.values():
            if is_sub_rope(x_fact.fact_context, required_context_subrope):
                fact_context_keg = x_person.get_keg_obj(x_fact.fact_context)
