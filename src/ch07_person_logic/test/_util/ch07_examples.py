...
def get_personunit_with_4_levels() -> PersonUnit:
    ...
    sue_person.set_l1_keg(kegunit_shop(exx.casa, star=30, pledge=True))
    ...
    sue_person.set_l1_keg(keg_kid_sem_jours)
    sun_str = "Sun"
    mon_str = "Mon"
    tue_str = "Tue"
    thu_str = "Thur"
    fri_str = "Fri"
    sat_str = "Sat"
    sue_person.set_keg_obj(kegunit_shop(sun_str, star=20), wk_rope)
    sue_person.set_keg_obj(kegunit_shop(mon_str, star=20), wk_rope)
    sue_person.set_keg_obj(kegunit_shop(tue_str, star=20), wk_rope)
    sue_person.set_keg_obj(kegunit_shop(exx.wed, star=20), wk_rope)
    sue_person.set_keg_obj(kegunit_shop(thu_str, star=30), wk_rope)
    sue_person.set_keg_obj(kegunit_shop(fri_str, star=40), wk_rope)
    sue_person.set_keg_obj(kegunit_shop(sat_str, star=50), wk_rope)

    nation_str = "nation"
    nation_rope = sue_person.make_l1_rope(nation_str)
    keg_kid_nation = kegunit_shop(nation_str, star=30)
    sue_person.set_l1_keg(keg_kid_nation)
    usa_str = "USA"
    ...
    sue_person.set_keg_obj(keg_grandkid_france, nation_rope)
    sue_person.set_keg_obj(keg_grandkid_brazil, nation_rope)
    sue_person.set_keg_obj(keg_grandkid_usa, nation_rope)
    texas_str = "Texas"
    oregon_str = "Oregon"
    keg_grandgrandkid_usa_texas = kegunit_shop(texas_str, star=50)
    keg_grandgrandkid_usa_oregon = kegunit_shop(oregon_str, star=50)
    sue_person.set_keg_obj(keg_grandgrandkid_usa_texas, usa_rope)
    sue_person.set_keg_obj(keg_grandgrandkid_usa_oregon, usa_rope)
    return sue_person
...
def get_personunit_with_4_levels_and_2reasons() -> PersonUnit:
    ...
    sue_person.edit_keg_attr(casa_rope, reason=wk_reason)
    sue_person.edit_keg_attr(casa_rope, reason=nation_reason)
    return sue_person
...
def get_personunit_with7am_clean_table_reason() -> PersonUnit:
    ...
    sue_person.set_keg_obj(x24hr_keg, ziet_rope)
    sue_person.set_keg_obj(am_keg, x24hr_rope)
    sue_person.set_keg_obj(pm_keg, x24hr_rope)
    sue_person.set_keg_obj(n1_keg, am_rope)  # keg_am
    sue_person.set_keg_obj(n2_keg, am_rope)  # keg_am
    sue_person.set_keg_obj(n3_keg, am_rope)  # keg_am
    ...
    sue_person.set_l1_keg(house_keg)
    sue_person.set_keg_obj(clean_keg, house_rope)
    sue_person.set_keg_obj(dish_keg, clean_rope)
    sue_person.set_keg_obj(soap_keg, clean_rope)
    sue_person.set_keg_obj(grab_keg, soap_rope)
    ...
    sue_person.edit_keg_attr(clean_rope, reason=clean_table_7am_reason)
    casa_rope = sue_person.make_l1_rope(exx.casa)
    sue_person.edit_keg_attr(casa_rope, reason=clean_table_7am_reason)
    return sue_person
...
def get_personunit_1task_1ceo_minutes_reason_1fact() -> PersonUnit:
    ...
    yao_person.set_l1_keg(hr_min_keg)
    ...
    yao_person.set_l1_keg(mail_keg)
    yao_person.edit_keg_attr(mail_rope, reason=hr_reasonunit)
    return yao_person
...
def get_personunit_x1_3levels_1reason_1facts() -> PersonUnit:
    ...
    zia_person.set_keg_obj(keg_grandkidU, wk_rope)
    zia_person.set_keg_obj(keg_grandkidM, wk_rope)

    shave_reason = reasonunit_shop(wk_rope)
    shave_reason.set_case(mon_rope)

    zia_person.edit_keg_attr(shave_rope, reason=shave_reason)
    ...
    zia_person.edit_keg_attr(shave_rope, factunit=x_factunit)
    return zia_person


def get_personunit_reason_context_ziet_example() -> PersonUnit:
    sue_person = personunit_shop("Sue")
    sue_person.set_l1_keg(kegunit_shop("casa"))
    return sue_person


def get_personunit_irrational_example() -> PersonUnit:
    # sourcery skip: extract-duplicate-method
    # this person has no definitive agenda because 2 pledge kegs are in contradiction
    ...
    hatter_person.set_l1_keg(kegunit_shop(egg_str))

    chicken_str = "chicken first"
    chicken_rope = hatter_person.make_l1_rope(chicken_str)
    hatter_person.set_l1_keg(kegunit_shop(chicken_str))

    # set egg pledge is True when chicken first is False
    hatter_person.edit_keg_attr(
        egg_rope,
        pledge=True,
...
def get_mop_with_reason_personunit_example1():
    ...
    sue_person.set_keg_obj(kegunit_shop(exx.clean), situation_rope)
    sue_person.set_keg_obj(kegunit_shop("very_much"), clean_rope)
    sue_person.set_keg_obj(kegunit_shop("moderately"), clean_rope)
    sue_person.set_keg_obj(kegunit_shop("dirty"), situation_rope)

    floor_reason = reasonunit_shop(situation_rope)
    floor_reason.set_case(case=situation_rope)
    sue_person.edit_keg_attr(floor_rope, reason=floor_reason)
    return sue_person
...
def get_personunit_laundry_example1() -> PersonUnit:
    ...
    yao_person.set_keg_obj(kegunit_shop(basket_str), casa_rope)
    yao_person.set_keg_obj(kegunit_shop(b_full_str), basket_rope)
    yao_person.set_keg_obj(kegunit_shop(b_smel_str), basket_rope)
    yao_person.set_keg_obj(kegunit_shop(b_bare_str), basket_rope)
    yao_person.set_keg_obj(kegunit_shop(b_fine_str), basket_rope)
    yao_person.set_keg_obj(kegunit_shop(b_half_str), basket_rope)
    yao_person.set_keg_obj(kegunit_shop(do_laundry_str, pledge=True), casa_rope)
    ...
    yao_person.edit_keg_attr(laundry_task_rope, laborunit=cali_laborunit)
