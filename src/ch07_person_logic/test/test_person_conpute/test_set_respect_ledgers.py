...
def test_PersonUnit_set_partnerunit_groupunit_respect_ledgers_SetsAttr_Scenario0():
    # ESTABLISH
    sue_personunit = personunit_shop(exx.sue)
    assert sue_personunit.groupunits == {}

    # WHEN
    sue_personunit._set_partnerunit_groupunit_respect_ledgers()

    # THEN
    assert sue_personunit.groupunits == {}


def test_PersonUnit_set_partnerunit_groupunit_respect_ledgers_Clears_groupunits():
    # ESTABLISH
    sue_personunit = personunit_shop(exx.sue)
...
def test_PersonUnit_set_partnerunit_groupunit_respect_ledgers_SetsAttr_Scenario1():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    sue_personunit = personunit_shop(exx.sue)
    sue_personunit.add_partnerunit(exx.yao)
    ...
    assert yao_partnerunit.credor_pool != 0
    assert yao_partnerunit.debtor_pool != 0
    assert yao_partnerunit.credor_pool == sue_personunit.credor_respect
    assert yao_partnerunit.debtor_pool == sue_personunit.debtor_respect
...
def test_PersonUnit_set_partnerunit_groupunit_respect_ledgers_SetsAttr_Scenario2():
    # ESTABLISH
    sue_personunit = personunit_shop(exx.sue)
    sue_personunit.add_partnerunit(exx.yao)
    yao_partnerunit = sue_personunit.get_partner(exx.yao)
    ...
    # THEN
    assert sue_personunit.get_partner(exx.yao).credor_pool != 0
    assert sue_personunit.get_partner(exx.yao).debtor_pool != 0
    assert yao_partnerunit.get_membership(exx.yao).credor_pool != 0
    assert yao_partnerunit.get_membership(exx.yao).debtor_pool != 0
    yao_yao_membership = yao_partnerunit.get_membership(exx.yao)
...
def test_PersonUnit_set_partnerunit_groupunit_respect_ledgers_ResetPartnerUnitsAttrs():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    sue_personunit = personunit_shop(exx.sue)
    sue_personunit.add_partnerunit(exx.yao, 55, 55)
    sue_personunit.add_partnerunit(exx.zia, 55, 55)
    yao_partnerunit = sue_personunit.get_partner(exx.yao)
    zia_partnerunit = sue_personunit.get_partner(exx.zia)
