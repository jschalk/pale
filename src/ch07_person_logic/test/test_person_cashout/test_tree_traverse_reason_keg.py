...
def test_agenda_returned_WhenNoReasonsExist():
    ...
    assert sue_person.get_keg_obj(casa_rope).task is True
    cat_rope = sue_person.make_l1_rope("cat have dinner")
    assert sue_person.get_keg_obj(cat_rope).task is True
...
def test_PersonUnit_reasonheirs_AreInherited_v1():
    ...
    sue_person.edit_keg_attr(casa_rope, reason=casa_wk_build_reasonunit)
...
def test_PersonUnit_reasonheirs_AreInheritedTo4LevelsFromRoot():
    ...
    a4_person.edit_keg_attr(casa_rope, reason=casa_wk_build_reasonunit)

    # WHEN
    rla_str = "hp"
    rla_rope = a4_person.make_rope(casa_rope, rla_str)
    a4_person.set_keg_obj(kegunit_shop(rla_str), parent_rope=rla_rope)
    cost_str = "cost_quantification"
    cost_rope = a4_person.make_rope(rla_rope, cost_str)
    a4_person.set_keg_obj(kegunit_shop(cost_str), parent_rope=cost_rope)
    a4_person.cashout()

    # THEN
    casa_keg = a4_person.kegroot.kids[exx.casa]
...
def test_PersonUnit_reasonheirs_AreInheritedTo4LevelsFromLevel2():
    ...
    wk_rope = a4_person.make_l1_rope(wk_keg_label)
    wed_rope = a4_person.make_rope(wk_rope, exx.wed)
    ...
    casa_keg = a4_person.kegroot.get_kid(exx.casa)
    rla_keg = casa_keg.get_kid(rla_str)
    cost_keg = rla_keg.get_kid(cost_str)

    assert a4_person.kegroot.reasonheirs == {}
    ...
    # THEN
    assert a4_person.kegroot.reasonheirs == {}  # casa_wk_built_reasonheir
...
def test_PersonUnit_ReasonUnits_set_UnCoupledMethod():
    ...
    # WHEN
    sue_person.edit_keg_attr(casa_rope, reason_context=wk_rope, reason_case=wed_rope)

    # THEN
    casa_keg1 = sue_person.get_keg_obj(casa_rope)
...
def test_PersonUnit_ReasonUnits_set_caseKegWithDenomSetsCaseDivision():
    # ESTABLISH
    sue_person = get_personunit_with_4_levels()
    casa_rope = sue_person.make_l1_rope(exx.casa)
    ziet_str = "ziet"
    ...
    sue_person.set_l1_keg(kegunit_shop(ziet_str, begin=100, close=2000))
    sue_person.set_keg_obj(kegunit_shop(exx.wk, denom=7), parent_rope=ziet_rope)
...
def test_PersonUnit_ReasonUnits_set_caseKegWithBeginCloseSetsCasereason_lower_reason_upper():
    # ESTABLISH
    sue_person = get_personunit_with_4_levels()
    casa = "casa"
    casa_rope = sue_person.make_l1_rope(casa)
    ...
    sue_person.set_keg_obj(kegunit_shop(ziet, begin=100, close=2000), root_rope)
    sue_person.set_keg_obj(kegunit_shop(rus_war, begin=22, close=34), ziet_rope)
...
def test_PersonUnit_ReasonUnits_edit_keg_attr_Deletes_ReasonUnits_And_CaseUnits():
    # ESTABLISH
    sue_person = get_personunit_with_4_levels()
    casa_rope = sue_person.make_l1_rope("casa")
...
def test_PersonUnit_edit_keg_attr_personIsAbleToEdit_active_requisite_AnyKegIfInvaildThrowsError():
    ...
    root_rope = sue_person.kegroot.get_keg_rope()
    sue_person.set_keg_obj(kegunit_shop(run_str), root_rope)
    sue_person.cashout()  # set tree metrics
    run_keg = sue_person.get_keg_obj(run_rope)
...
def test_PersonUnit_ReasonUnits_KegUnit_active_InfluencesReasonUnit_reason_active():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH an Person with 5 kegs, 1 Fact:
...
def test_PersonUnit_tree_traverse_count_SetByTotalNumberOfTreeTraversesEndsIsDetected():
    ...
    # WHEN
    sue_person.cashout()
    # for keg_key in sue_person._keg_dict.keys():
    #     print(f"{keg_key=}")
