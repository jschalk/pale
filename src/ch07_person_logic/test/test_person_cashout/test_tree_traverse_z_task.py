...
def test_PersonUnit_cashout_Sets_active_WhenFactSaysNo():
    ...
    # for keg in sue_personunit._keg_dict.values():
    #     print(f"{casa_rope=} {keg.get_keg_rope()=}")
    casa_rope = sue_personunit.make_l1_rope(exx.casa)
    assert sue_personunit.get_keg_obj(casa_rope).keg_active is None
    ...
    # THEN
    assert sue_personunit._keg_dict != {}
    assert len(sue_personunit._keg_dict) == 17
    # for keg in sue_personunit._keg_dict.values():
    #     print(f"{casa_rope=} {keg.get_keg_rope()=}")
    assert sue_personunit.get_keg_obj(casa_rope).keg_active is False
...
def test_PersonUnit_cashout_Sets_active_WhenFactModifies():
    ...
    assert sue_personunit._keg_dict
    assert len(sue_personunit._keg_dict) == 17
    assert sue_personunit._keg_dict.get(casa_rope).keg_active is False
    ...
    assert sue_personunit._keg_dict
    assert len(sue_personunit._keg_dict) == 17
    assert sue_personunit._keg_dict.get(casa_rope).keg_active
...
def test_PersonUnit_cashout_Sets_keg_dict():
    # ESTABLISH
    sue_personunit = get_personunit_with_4_levels_and_2reasons()
    wk_str = "sem_jours"
    wk_rope = sue_personunit.make_l1_rope(wk_str)
    ...
    casa_rope = sue_personunit.make_l1_rope(exx.casa)
    casa_keg = sue_personunit.get_keg_obj(casa_rope)
    print(f"{sue_personunit.person_name=} {len(casa_keg.reasonunits)=}")
    # print(f"{casa_keg.reasonunits=}")
    print(f"{sue_personunit.person_name=} {len(sue_personunit.kegroot.factunits)=}")
    # print(f"{sue_personunit.kegroot.factunits=}")
    ...
    # THEN
    casa_keg = sue_personunit._keg_dict.get(casa_rope)
    print(f"\nlook at {casa_keg.get_keg_rope()=}")
    assert casa_keg.parent_rope == sue_personunit.kegroot.get_keg_rope()
...
def test_PersonUnit_cashout_CalculatesRangeAttributes():
    ...
    assert sue_personunit._keg_dict.get(clean_rope).keg_active is False
    ...
    # THEN
    sue_personunit.cashout()
    assert sue_personunit._keg_dict.get(clean_rope).keg_active

    # WHEN
    # set facts as 8am to 10am
    x24hr_reason_lower = 8.0
    x24hr_reason_upper = 10.0
    print(sue_personunit.kegroot.factunits[x24hr_rope])
    sue_personunit.add_fact(
        x24hr_reason_context,
        fact_state=x24hr_fact_state,
        fact_lower=x24hr_reason_lower,
    ...
    print(sue_personunit.kegroot.factunits[x24hr_rope])
    print(sue_personunit.kegroot.kids[house_str].kids[clean_str].reasonunits)
    # sue_personunit.kegroot.kids["houseadministration"].kids[clean_str].keg_active = None
...
def test_PersonUnit_get_agenda_dict_ReturnsObj_WithSinglePledge():
    ...
    # WHEN
    pledge_kegs = sue_personunit.get_agenda_dict()
...
def test_PersonUnit_cashout_SetsData_personunit_v001():
    ...
    # print(f"{len(yao_personunit.kegroot.kids)=}")
    ulty_str = "Ultimate Frisbee"
    ulty_rope = yao_personunit.make_l1_rope(ulty_str)

    # if yao_personunit.kegroot.kids["Ultimate Frisbee"].keg_label == "Ultimate Frisbee":
    assert yao_personunit.kegroot.kids[ulty_str].reasonunits is not None
    assert yao_personunit.person_name is not None

    # for fact in yao_personunit.kegroot.factunits.values():
    #     print(f"{fact=}")
    ...
    assert yao_personunit._keg_dict.get(laundry_rope).keg_active is False
...
def test_PersonUnit_cashout_OptionWeekJoursReturnsObj_personunit_v001():
    ...
    x_kegroot = yao_personunit.kegroot
    ...
    # WHEN
    keg_dict = yao_personunit.get_keg_dict()
...
def test_PersonUnit_cashout_SetsKegUnitsActiveWithEvery6WeeksReason_personunit_v001():
    # ESTABLISH
    yao_personunit = personunit_v001()
    ...
    reason_lower = None
    reason_upper = None
    print(f"{len(yao_personunit._keg_dict)=}")
    ...
    clean_sheet_keg = yao_personunit.get_keg_obj(clean_couch_rope)
    ...
    print(f"{yao_personunit.kegroot.factunits=}")
    yao_personunit.cashout()
    ...
    clean_couch_keg = yao_personunit.get_keg_obj(rope=clean_couch_rope)
...
def test_PersonUnit_cashout_SetsAttr_KegUnits_active_personunit_v001():
    # ESTABLISH
    yao_personunit = personunit_v001()
    ...
    assert len(yao_personunit._keg_dict) == sum(
        keg.keg_active is not None for keg in yao_personunit._keg_dict.values()
    )
...
def test_PersonUnit_cashout_EveryTwoMonthReturnsObj_personunit_v001():
    ...
    mat_rope = yao_personunit.make_rope(clean_rope, mat_keg_label)
    ...
    # THEN
    print(f"{len(keg_dict)=}")
    print(f"{len(yao_personunit.kegroot.factunits)=}")
    assert from_list_get_active(mat_rope, yao_personunit._keg_dict)


def test_PersonUnit_cashout_SetsEmpty_sum_healerunit_kegs_fund_total():
    # ESTABLISH
    sue_personunit = personunit_shop("Sue")
    assert sue_personunit.sum_healerunit_kegs_fund_total == 0
...
def test_PersonUnit_cashout_Sets_sum_healerunit_kegs_fund_total(graphics_bool):
    # ESTABLISH
    sue_personunit = get_personunit_with_4_levels_and_2reasons()
    ...
    oregon_keg = sue_personunit.get_keg_obj(oregon_rope)
    ...
    # THEN
    assert (
        sue_personunit.sum_healerunit_kegs_fund_total
    ...
    sue_personunit.edit_keg_attr(wk_rope, problem_bool=True)
    mon_rope = sue_personunit.make_rope(wk_rope, "Mon")
    sue_personunit.edit_keg_attr(mon_rope, healerunit=sue_healerunit)
    mon_keg = sue_personunit.get_keg_obj(mon_rope)
    ...
    assert (
        sue_personunit.sum_healerunit_kegs_fund_total == 0.06923077 * default_pool_num()
    )
    ...
    sue_personunit.edit_keg_attr(tue_rope, healerunit=sue_healerunit)
    tue_keg = sue_personunit.get_keg_obj(tue_rope)
    # print(f"{tue_keg.problem_bool=} {tue_keg.fund_ratio=}")
    # sat_rope = sue_personunit.make_rope(wk_rope, "Sat")
    # sat_keg = sue_personunit.get_keg_obj(sat_rope)
    ...
    # WHEN
    sue_personunit.edit_keg_attr(wk_rope, healerunit=sue_healerunit)
    wk_keg = sue_personunit.get_keg_obj(wk_rope)
    print(f"{wk_keg.keg_label=} {wk_keg.problem_bool=} {wk_keg.fund_ratio=}")
    sue_personunit.cashout()
    # THEN
    display_kegtree(sue_personunit, "Keep", graphics_bool)
    assert sue_personunit.sum_healerunit_kegs_fund_total == 0
    assert oregon_keg.healerunit_ratio == 0
...
def test_PersonUnit_cashout_Sets_healers_dict():
    ...
    sue_personunit.edit_keg_attr(wk_rope, problem_bool=True, healerunit=bob_healerunit)
    assert sue_personunit._healers_dict == {}
    ...
    wk_keg = sue_personunit.get_keg_obj(wk_rope)
    assert sue_personunit._healers_dict.get(exx.bob) == {wk_rope: wk_keg}
    oregon_keg = sue_personunit.get_keg_obj(oregon_keep_rope)
    assert sue_personunit._healers_dict.get(exx.sue) == {oregon_keep_rope: oregon_keg}
...
def test_PersonUnit_cashout_Sets_keeps_buildable_False():
    ...
    sue_personunit.set_keg_obj(kegunit_shop(bend_str), oregon_rope)
    sue_healerunit = healerunit_shop({exx.sue})
    sue_personunit.edit_keg_attr(
