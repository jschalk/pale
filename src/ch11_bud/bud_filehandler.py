...
def collect_plan_spark_dir_sets(
    moment_mstr_dir: str, moment_rope: RopeTerm
) -> dict[PlanName, set[SparkInt]]:
    x_dict = {}
    plans_dir = create_moment_plans_dir_path(moment_mstr_dir, moment_rope)
    set_dir(plans_dir)
...
def save_arbitrary_planspark(
    ...
    x_planunit = planunit_shop(plan_name, moment_rope)
...
def cellunit_add_json_file(
    ...
    cell_dir = create_cell_dir_path(
        moment_mstr_dir, moment_rope, time_plan_name, bud_time, bud_ancestors
    )
...
def save_bud_file(
    ...
    bud_json_path = create_budunit_json_path(
        moment_mstr_dir, moment_rope, plan_name, x_bud.bud_time
    )
...
def bud_file_exists(
    ...
    bud_json_path = create_budunit_json_path(
        moment_mstr_dir, moment_rope, plan_name, x_bud_time
    )
...
def open_bud_file(
    ...
    if bud_file_exists(moment_mstr_dir, moment_rope, plan_name, x_bud_time):
...
def get_timenum_dirs(
    ...
    buds_dir = create_buds_dir_path(moment_mstr_dir, moment_rope, plan_name)
