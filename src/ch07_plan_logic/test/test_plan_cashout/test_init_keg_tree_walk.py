...
def test_PlanUnit_set_keg_dict_Scenario0():
    # ESTABLISH
    ...
    assert not root_keg.gogo_calc
    assert not root_keg.stop_calc
    assert yao_plan._keg_dict == {root_keg.get_keg_rope(): root_keg}
    assert yao_plan.reason_contexts == set()


def test_PlanUnit_set_keg_dict_Scenario1():
    # ESTABLISH
    yao_plan = planunit_shop("Yao")
    ziet0_begin = 7
    ziet0_close = 31
...
def test_PlanUnit_set_keg_dict_Clears_gogo_calc_stop_calc():
    # ESTABLISH
...
def test_PlanUnit_set_keg_dict_Sets_reason_contexts():
    # ESTABLISH
    ...
    sue_plan.add_keg(polis_rope)
    sue_plan.add_keg(nation_rope)
    ...
    # THEN
    assert sue_plan.reason_contexts == {polis_rope}


def test_PlanUnit_set_keg_CreatesKegUnitsUsedBy_reasonunits():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    ...
    cuisine_room_rope = sue_plan.make_rope(buildings_rope, cuisine_room_str)
    ...
    assert sue_plan.keg_exists(buildings_rope) is False

    # WHEN
    sue_plan.set_keg_obj(clean_cuisine_keg, cleaning_rope, create_missing_kegs=True)

    # THEN
    assert sue_plan.keg_exists(buildings_rope)
...
def test_get_sorted_keg_list_ReturnsObj_Scenario0_DefaultOrder_keg_keg_rope():
    ...
    # WHEN
    x_sorted_keg_list = get_sorted_keg_list(sue_plan._keg_dict)

    # THEN
    assert x_sorted_keg_list is not None
    assert len(x_sorted_keg_list) == 17
    assert x_sorted_keg_list[0] == sue_plan.kegroot
    assert x_sorted_keg_list[1] == sue_plan.get_keg_obj(casa_rope)
    assert x_sorted_keg_list[11] == sue_plan.get_keg_obj(mon_rope)
