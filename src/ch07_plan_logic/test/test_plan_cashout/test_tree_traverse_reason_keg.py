...
def test_agenda_returned_WhenNoReasonsExist():
    ...
    assert sue_plan.get_keg_obj(casa_rope).task is True
    cat_rope = sue_plan.make_l1_rope("cat have dinner")
    assert sue_plan.get_keg_obj(cat_rope).task is True


def test_PlanUnit_reasonheirs_AreInherited_v1():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    casa_rope = sue_plan.make_l1_rope("casa")
    ...
    sue_plan.edit_keg_attr(casa_rope, reason=casa_wk_build_reasonunit)
...
def test_PlanUnit_reasonheirs_AreInheritedTo4LevelsFromRoot():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    a4_plan = get_planunit_with_4_levels()
    casa_rope = a4_plan.make_l1_rope(exx.casa)
    wk_str = "sem_jours"
    wk_rope = a4_plan.make_l1_rope(wk_str)
    wed_rope = a4_plan.make_rope(wk_rope, exx.wed)
    ...
    a4_plan.edit_keg_attr(casa_rope, reason=casa_wk_build_reasonunit)

    # WHEN
    rla_str = "hp"
    rla_rope = a4_plan.make_rope(casa_rope, rla_str)
    a4_plan.set_keg_obj(kegunit_shop(rla_str), parent_rope=rla_rope)
    cost_str = "cost_quantification"
    cost_rope = a4_plan.make_rope(rla_rope, cost_str)
    a4_plan.set_keg_obj(kegunit_shop(cost_str), parent_rope=cost_rope)
    a4_plan.cashout()

    # THEN
    casa_keg = a4_plan.kegroot.kids[exx.casa]
...
def test_PlanUnit_reasonheirs_AreInheritedTo4LevelsFromLevel2():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    a4_plan = get_planunit_with_4_levels()
    casa_rope = a4_plan.make_l1_rope(exx.casa)
    wk_keg_label = "sem_jours"
    wk_rope = a4_plan.make_l1_rope(wk_keg_label)
    wed_rope = a4_plan.make_rope(wk_rope, exx.wed)
    ...
    casa_keg = a4_plan.kegroot.get_kid(exx.casa)
    rla_keg = casa_keg.get_kid(rla_str)
    cost_keg = rla_keg.get_kid(cost_str)

    assert a4_plan.kegroot.reasonheirs == {}
    ...
    # THEN
    assert a4_plan.kegroot.reasonheirs == {}  # casa_wk_built_reasonheir
...
def test_PlanUnit_ReasonUnits_set_UnCoupledMethod():
    ...
    # WHEN
    sue_plan.edit_keg_attr(casa_rope, reason_context=wk_rope, reason_case=wed_rope)

    # THEN
    casa_keg1 = sue_plan.get_keg_obj(casa_rope)
...
def test_PlanUnit_ReasonUnits_set_caseKegWithDenomSetsCaseDivision():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    casa_rope = sue_plan.make_l1_rope(exx.casa)
    ziet_str = "ziet"
    ziet_rope = sue_plan.make_l1_rope(ziet_str)
    wk_rope = sue_plan.make_rope(ziet_rope, exx.wk)
    sue_plan.set_l1_keg(kegunit_shop(ziet_str, begin=100, close=2000))
    sue_plan.set_keg_obj(kegunit_shop(exx.wk, denom=7), parent_rope=ziet_rope)
...
def test_PlanUnit_ReasonUnits_set_caseKegWithBeginCloseSetsCasereason_lower_reason_upper():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    casa = "casa"
    casa_rope = sue_plan.make_l1_rope(casa)
    ziet = "ziet"
    ziet_rope = sue_plan.make_l1_rope(ziet)
    rus_war = "rus_war"
    rus_war_rope = sue_plan.make_rope(ziet_rope, rus_war)
    root_rope = sue_plan.kegroot.get_keg_rope()
    sue_plan.set_keg_obj(kegunit_shop(ziet, begin=100, close=2000), root_rope)
    sue_plan.set_keg_obj(kegunit_shop(rus_war, begin=22, close=34), ziet_rope)
...
def test_PlanUnit_ReasonUnits_edit_keg_attr_Deletes_ReasonUnits_And_CaseUnits():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    casa_rope = sue_plan.make_l1_rope("casa")
    sem_jour_rope = sue_plan.make_l1_rope("sem_jours")
    wed_rope = sue_plan.make_rope(sem_jour_rope, "Wed")

    sue_plan.edit_keg_attr(
        casa_rope, reason_context=sem_jour_rope, reason_case=wed_rope
    )
    thu_rope = sue_plan.make_rope(sem_jour_rope, "Thur")
    sue_plan.edit_keg_attr(
        casa_rope,
...
def test_PlanUnit_ReasonUnits_del_reason_case_UncoupledMethod2():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    casa_rope = sue_plan.make_l1_rope("casa")
...
def test_PlanUnit_edit_keg_attr_planIsAbleToEdit_active_requisite_AnyKegIfInvaildThrowsError():
    ...
    root_rope = sue_plan.kegroot.get_keg_rope()
    sue_plan.set_keg_obj(kegunit_shop(run_str), root_rope)
    sue_plan.cashout()  # set tree metrics
    run_keg = sue_plan.get_keg_obj(run_rope)
...
def test_PlanUnit_ReasonUnits_KegUnit_active_InfluencesReasonUnit_reason_active():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH an Plan with 5 kegs, 1 Fact:
    ...
    sem_jours_rope = sue_plan.make_l1_rope(sem_jours_str)
    wed_rope = sue_plan.make_rope(sem_jours_rope, exx.wed)
    thu_str = "Thur"
    thu_rope = sue_plan.make_rope(sem_jours_rope, thu_str)
    ...
    sue_plan.add_fact(fact_context=sem_jours_rope, fact_state=wed_rope)
    sue_plan.cashout()
    ...
    sue_plan.add_fact(fact_context=sem_jours_rope, fact_state=thu_rope)
...
def test_PlanUnit_cashout_SetsRationalAttrToFalseWhen_max_tree_traverse_Is1():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    assert sue_plan.rational is False
    # sue_plan.cashout()
    sue_plan.rational = True
    assert sue_plan.rational
...
def test_PlanUnit_tree_traverse_count_SetByTotalNumberOfTreeTraversesEndsIsDetected():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    ...
    # WHEN
    sue_plan.cashout()
    # for keg_key in sue_plan._keg_dict.keys():
    #     print(f"{keg_key=}")

    # THEN
    assert sue_plan.tree_traverse_count == 2


def test_PlanUnit_tree_traverse_count_CountsTreeTraversesForIrrationalPlans():
    # ESTABLISH irrational plan
    sue_plan = get_planunit_irrational_example()
    sue_plan.cashout()
    assert sue_plan.tree_traverse_count == 3
    ...
    # THEN
    assert sue_plan.tree_traverse_count == 21
