...
def test_create_groupunits_metrics_SetsAttrScenario1():
    ...
    ohio_str = ";Ohio"
    yao_personunit.add_membership(ohio_str)
    yao_yao_membership = yao_personunit.get_membership(exx.yao)
    yao_ohio_membership = yao_personunit.get_membership(ohio_str)
...
def test_PlanUnit_set_personunit_groupunit_respect_ledgers_SetsAttr_Scenario0():
    # ESTABLISH
    sue_planunit = planunit_shop(exx.sue)
    assert sue_planunit.groupunits == {}

    # WHEN
    sue_planunit._set_personunit_groupunit_respect_ledgers()

    # THEN
    assert sue_planunit.groupunits == {}


def test_PlanUnit_set_personunit_groupunit_respect_ledgers_Clears_groupunits():
    # ESTABLISH
    sue_planunit = planunit_shop(exx.sue)
...
def test_PlanUnit_set_personunit_groupunit_respect_ledgers_SetsAttr_Scenario1():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    sue_planunit = planunit_shop(exx.sue)
    sue_planunit.add_personunit(exx.yao)
    ...
    assert yao_personunit.get_membership(exx.yao).credor_pool == 0
    assert yao_personunit.get_membership(exx.yao).debtor_pool == 0
    # assert sue_planunit.groupunits == {}
    ...
    assert yao_personunit.credor_pool != 0
    assert yao_personunit.debtor_pool != 0
    assert yao_personunit.credor_pool == sue_planunit.credor_respect
    assert yao_personunit.debtor_pool == sue_planunit.debtor_respect
    yao_membership = yao_personunit.get_membership(exx.yao)
    ...
    yao_groupunit = sue_planunit.get_groupunit(exx.yao)
    groupunit_yao_membership = yao_groupunit.get_person_membership(exx.yao)
    assert yao_membership == groupunit_yao_membership


def test_PlanUnit_set_personunit_groupunit_respect_ledgers_SetsAttr_Scenario2():
    # ESTABLISH
    sue_planunit = planunit_shop(exx.sue)
    sue_planunit.add_personunit(exx.yao)
    yao_personunit = sue_planunit.get_person(exx.yao)
    yao_personunit.add_membership(exx.yao, 1, 4)
    ohio_str = ";Ohio"
    yao_personunit.add_membership(ohio_str, 3, 1)
    assert yao_personunit.credor_pool == 0
    assert yao_personunit.debtor_pool == 0
    assert yao_personunit.get_membership(exx.yao).credor_pool == 0
    assert yao_personunit.get_membership(exx.yao).debtor_pool == 0
    ...
    # THEN
    assert sue_planunit.get_person(exx.yao).credor_pool != 0
    assert sue_planunit.get_person(exx.yao).debtor_pool != 0
    assert yao_personunit.get_membership(exx.yao).credor_pool != 0
    assert yao_personunit.get_membership(exx.yao).debtor_pool != 0
    yao_yao_membership = yao_personunit.get_membership(exx.yao)
...
def test_PlanUnit_set_personunit_groupunit_respect_ledgers_ResetPersonUnitsAttrs():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    sue_planunit = planunit_shop(exx.sue)
    sue_planunit.add_personunit(exx.yao, 55, 55)
    sue_planunit.add_personunit(exx.zia, 55, 55)
    yao_personunit = sue_planunit.get_person(exx.yao)
    zia_personunit = sue_planunit.get_person(exx.zia)
    yao_personunit.add_person_fund_give_take(0.5, 0.6, 0.1, 0.22)
    zia_personunit.add_person_fund_give_take(0.2, 0.1, 0.1, 0.22)
    zia_1 = 0.8
    zia_2 = 0.5
    zia_3 = 200
    zia_4 = 140
    zia_personunit.set_fund_agenda_ratio_give_take(
        fund_agenda_ratio_give_sum=zia_1,
        fund_agenda_ratio_take_sum=zia_2,
        personunits_person_cred_lumen_sum=zia_3,
        personunits_person_debt_lumen_sum=zia_4,
    )
    yao_1 = 0.2
    yao_2 = 0.5
    yao_3 = 204
    yao_4 = 144
    yao_personunit.set_fund_agenda_ratio_give_take(
        fund_agenda_ratio_give_sum=yao_1,
        fund_agenda_ratio_take_sum=yao_2,
        personunits_person_cred_lumen_sum=yao_3,
        personunits_person_debt_lumen_sum=yao_4,
    )
    assert zia_personunit.fund_agenda_ratio_give == 0.125
    assert zia_personunit.fund_agenda_ratio_take == 0.44
    assert yao_personunit.fund_agenda_ratio_give == 0.5
    assert yao_personunit.fund_agenda_ratio_take == 0.44

    # WHEN
    sue_planunit._set_personunit_groupunit_respect_ledgers()

    # THEN
    assert zia_personunit.fund_agenda_ratio_give == 0
    assert zia_personunit.fund_agenda_ratio_take == 0
