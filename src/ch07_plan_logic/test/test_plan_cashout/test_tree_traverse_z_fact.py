...
from src.ch07_plan_logic.test._util.ch07_examples import (
    get_planunit_1task_1ceo_minutes_reason_1fact,
)
from src.ref.keywords import ExampleStrs as exx


def test_PlanUnit_cashout_ChangesKegUnit_pledge_task():
    # ESTABLISH
    yao_plan = get_planunit_1task_1ceo_minutes_reason_1fact()
    hr_str = "hr"
    ...
    keg_dict = yao_plan.get_keg_dict()
    ...
    # THEN
    mail_keg = yao_plan.get_keg_obj(mail_rope)
    assert mail_keg.pledge
    assert mail_keg.task


def test_PlanUnit_cashout_ExecutesWithRangeRootFacts():
    # ESTABLISH
    ...
    zia_plan.set_keg_obj(clean_keg, parent_rope=casa_rope)
    zia_plan.add_fact(
        fact_context=clean_rope, fact_state=clean_rope, fact_lower=1, fact_upper=5
    )
    assert zia_plan.kegroot.factheirs == {}
    ...
    assert zia_plan.kegroot.factheirs == {clean_factheir.fact_context: clean_factheir}


def test_PlanUnit_cashout_RaisesErrorIfNon_RangeRootHasFactUnit():
    # ESTABLISH
    zia_plan = planunit_shop("Zia")
    casa_rope = zia_plan.make_l1_rope(exx.casa)
    zia_plan.set_l1_keg(kegunit_shop(exx.casa))
    ...
    zia_plan.set_keg_obj(clean_keg, parent_rope=casa_rope)
    zia_plan.set_keg_obj(sweep_keg, parent_rope=clean_rope)
    zia_plan.add_fact(sweep_rope, sweep_rope, fact_lower=1, fact_upper=5)
...
def test_PlanUnit_cashout_FactHeirsInherited():
    # ESTABLISH
    zia_plan = planunit_shop("Zia")
    swim_rope = zia_plan.make_l1_rope(exx.swim)
    zia_plan.set_l1_keg(kegunit_shop(exx.swim))
    fast_str = "fast"
    slow_str = "slow"
    fast_rope = zia_plan.make_rope(swim_rope, fast_str)
    slow_rope = zia_plan.make_rope(swim_rope, slow_str)
    zia_plan.set_keg_obj(kegunit_shop(fast_str), parent_rope=swim_rope)
    zia_plan.set_keg_obj(kegunit_shop(slow_str), parent_rope=swim_rope)

    earth_str = "earth"
    earth_rope = zia_plan.make_l1_rope(earth_str)
    zia_plan.set_l1_keg(kegunit_shop(earth_str))

    swim_keg = zia_plan.get_keg_obj(swim_rope)
    fast_keg = zia_plan.get_keg_obj(fast_rope)
    slow_keg = zia_plan.get_keg_obj(slow_rope)
    zia_plan.add_fact(
...
def test_PlanUnit_cashout_FactUnitMoldsFactHeir():
...
def test_PlanUnit_cashout_FactHeirDeletesFactUnit():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    ...
    fast_str = "fast"
    slow_str = "slow"
    sue_plan.set_keg_obj(kegunit_shop(fast_str), parent_rope=swim_rope)
    sue_plan.set_keg_obj(kegunit_shop(slow_str), parent_rope=swim_rope)
    earth_str = "earth"
    earth_rope = sue_plan.make_l1_rope(earth_str)
    sue_plan.set_l1_keg(kegunit_shop(earth_str))
    swim_keg = sue_plan.get_keg_obj(swim_rope)
    first_earthheir = factheir_shop(
        earth_rope, earth_rope, fact_lower=200.0, fact_upper=500.0
    )
    first_earthdict = {first_earthheir.fact_context: first_earthheir}
    sue_plan.add_fact(earth_rope, earth_rope, fact_lower=200.0, fact_upper=500.0)
    assert swim_keg.factheirs == {}
...
def test_PlanUnit_cashout_SetstaskAsComplete():
    # ESTABLISH
    yao_plan = get_planunit_1task_1ceo_minutes_reason_1fact()
    mail_str = "obtain mail"
    assert yao_plan is not None
    assert len(yao_plan.kegroot.kids[mail_str].reasonunits) == 1
    keg_dict = yao_plan.get_keg_dict()
    mail_keg = keg_dict.get(yao_plan.make_l1_rope(mail_str))
    hr_str = "hr"
    hr_rope = yao_plan.make_l1_rope(hr_str)
    yao_plan.add_fact(hr_rope, hr_rope, fact_lower=82, fact_upper=85)
    assert mail_keg.pledge
    assert mail_keg.task
