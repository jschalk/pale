...
def test_PlanUnit_cashout_WhenKegUnitHasFundsBut_kidsHaveNostarDistributeFundsToVoiceUnits_Scenario0():
    # ESTABLISH
    ...
    assert sue_planunit.get_voice(exx.yao).fund_give == 0
    assert sue_planunit.get_voice(exx.yao).fund_take == 0
    ...
    assert sue_planunit.get_voice(exx.yao).fund_give == default_pool_num()
    assert sue_planunit.get_voice(exx.yao).fund_take == default_pool_num()
...
def test_PlanUnit_cashout_TreeTraverseSetsAwardLine_fundFromRoot():
    ...
    sue_plan.add_voiceunit(voice_name=exx.sue)
...
def test_PlanUnit_cashout_WithRootLevelAwardUnitSetsGroupUnit_fund_give_fund_take():
    ...
    sue_plan.set_voiceunit(voiceunit_shop(exx.yao))
    sue_plan.set_voiceunit(voiceunit_shop(exx.zia))
    sue_plan.set_voiceunit(voiceunit_shop(exx.xio))
    ...
    assert len(sue_plan.get_voiceunit_group_titles_dict()) == 3
...
def test_PlanUnit_cashout_WithLevel3AwardUnitSetsGroupUnit_fund_give_fund_take():
    ...
    x_plan.set_voiceunit(voiceunit_shop(exx.yao))
    x_plan.set_voiceunit(voiceunit_shop(exx.zia))
    x_plan.set_voiceunit(voiceunit_shop(exx.xio))
    yao_awardunit = awardunit_shop(exx.yao, give_force=20, take_force=6)
    zia_awardunit = awardunit_shop(exx.zia, give_force=10, take_force=1)
    xio_awardunit = awardunit_shop(exx.xio, give_force=10)
    ...
    assert len(x_plan.get_voiceunit_group_titles_dict()) == 3
...
def test_PlanUnit_cashout_CreatesNewGroupUnitAndSetsGroup_fund_give_fund_take():
    ...
    swim_keg.set_awardunit(zia_awardunit)
    swim_keg.set_awardunit(xio_awardunit)
    assert len(x_plan.get_voiceunit_group_titles_dict()) == 2
    ...
    zia_groupunit = x_plan.get_groupunit(exx.zia)
    xio_groupunit = x_plan.get_groupunit(exx.xio)
    assert len(x_plan.get_voiceunit_group_titles_dict()) != len(x_plan.groupunits)
    assert yao_groupunit.fund_give == 0.5 * default_pool_num()
    assert yao_groupunit.fund_take == 0.75 * default_pool_num()
    assert zia_groupunit.fund_give == 0.25 * default_pool_num()
    assert zia_groupunit.fund_take == 0.125 * default_pool_num()
...
def test_PlanUnit_cashout_SetsGroupLinkPlanCredAndDebt():
    ...
    yao_plan.set_voiceunit(voiceunit_shop(exx.sue))
    yao_plan.set_voiceunit(voiceunit_shop(exx.bob))
    ...
    bob_voiceunit = yao_plan.get_voice(exx.bob)
    ...
    yao_plan.set_voiceunit(voiceunit_shop(exx.xio))
...
def test_PlanUnit_cashout_SetsVoiceUnitPlan_fund():
    # ESTABLISH
    yao_plan = planunit_shop("Yao")
    ...
    # THEN
    xio_voiceunit = yao_plan.get_voice(exx.xio)
...
def test_PlanUnit_cashout_SetsPartGroupedLWVoiceUnitPlan_fund():
    # ESTABLISH
    yao_plan = planunit_shop("Yao")
    swim_rope = yao_plan.make_l1_rope(exx.swim)
    yao_plan.set_l1_keg(kegunit_shop(exx.swim))
...
def test_PlanUnit_cashout_CreatesNewGroupUnitAndSetsVoice_fund_give_fund_take():
    # ESTABLISH
    bob_plan = planunit_shop(exx.bob)
    swim_rope = bob_plan.make_l1_rope(exx.swim)
    bob_plan.set_l1_keg(kegunit_shop(exx.swim))

    bob_plan.set_voiceunit(voiceunit_shop(exx.yao))
    bob_plan.set_voiceunit(voiceunit_shop(exx.zia))
    # bob_plan.set_voiceunit(voiceunit_shop(exx.xio))
    yao_awardunit = awardunit_shop(exx.yao, give_force=20, take_force=6)
    zia_awardunit = awardunit_shop(exx.zia, give_force=10, take_force=1)
    xio_awardunit = awardunit_shop(exx.xio, give_force=10)
    ...
    swim_keg.set_awardunit(zia_awardunit)
    swim_keg.set_awardunit(xio_awardunit)
    assert len(bob_plan.get_voiceunit_group_titles_dict()) == 2

    # WHEN
    bob_plan.cashout()

    # THEN
    assert len(bob_plan.get_voiceunit_group_titles_dict()) != len(bob_plan.groupunits)
    assert not bob_plan.voice_exists(exx.xio)
    yao_voiceunit = bob_plan.get_voice(exx.yao)
    zia_voiceunit = bob_plan.get_voice(exx.zia)
...
def test_PlanUnit_cashout_SetsVoiceUnit_fund_give_fund_take():
    # ESTABLISH
    yao_plan = planunit_shop("Yao")
    yao_plan.set_l1_keg(kegunit_shop("swim"))
    yao_plan.set_voiceunit(voiceunit_shop(exx.sue, 8))
    yao_plan.set_voiceunit(voiceunit_shop(exx.bob))
    yao_plan.set_voiceunit(voiceunit_shop(exx.zia))
    sue_voiceunit = yao_plan.get_voice(exx.sue)
    bob_voiceunit = yao_plan.get_voice(exx.bob)
...
def clear_all_voiceunits_groupunits_fund_agenda_give_take(x_plan: PlanUnit):
    ...
    # delete plan_agenda_debt and plan_agenda_cred
    for x_voiceunit in x_plan.voices.values():
...
class VoiceAgendaMetrics:
    ...
    def set_voiceagendametrics_sums(self, x_plan: PlanUnit):
...
def test_PlanUnit_agenda_cred_debt_SetAttrs():
    ...
    clear_all_voiceunits_groupunits_fund_agenda_give_take(yao_plan)
    ...
    x_voiceagendametrics.set_voiceagendametrics_sums(yao_plan)
    ...
    #     print(f"{keg_rope=}")
    # for x_voice in yao_plan.voices.values():
    ...
    assert all_voiceunits_have_legitimate_values(yao_plan)
...
def all_voiceunits_have_legitimate_values(x_plan: PlanUnit):
    return not any(
        (
...
def test_PlanUnit_cashout_SetsAttrsWhenNoFactUnitsNoReasonUnitsEmpty_agenda_ratio_cred_debt():
    ...
    yao_plan.set_voiceunit(sue_voiceunit)
    yao_plan.set_voiceunit(bob_voiceunit)
    yao_plan.set_voiceunit(zia_voiceunit)
    sue_voice = yao_plan.get_voice(exx.sue)
    bob_voice = yao_plan.get_voice(exx.bob)
