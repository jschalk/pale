...
from src.ch07_plan_logic.plan_tool import (
    ...
    get_plan_person_agenda_award_array,
    get_plan_person_agenda_award_csv,
)
from src.ref.keywords import ExampleStrs as exx


def test_get_plan_person_agenda_award_array_ReturnsObj_ScenarioZeroPersonUnits():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")

    # WHEN / THEN
    assert get_plan_person_agenda_award_array(sue_plan) == []


def test_get_plan_person_agenda_award_array_ReturnsObj_ScenarioSinglePersonUnit():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    sue_plan.add_personunit(exx.yao)

    # WHEN / THEN
    assert len(get_plan_person_agenda_award_array(sue_plan)) == 1


def test_get_plan_person_agenda_award_array_ReturnsObj_ScenarioMultiplePersonUnit():
    # ESTABLISH
    ...
    sue_plan.get_person(exx.yao).fund_agenda_give = yao_fund_agenda_give
    sue_plan.get_person(exx.yao).fund_agenda_take = yao_fund_agenda_take
    sue_plan.get_person(exx.bob).fund_agenda_give = bob_fund_agenda_give
    sue_plan.get_person(exx.bob).fund_agenda_take = bob_fund_agenda_take

    # WHEN
    plan_person_agenda_award_array = get_plan_person_agenda_award_array(sue_plan)

    # THEN
    assert len(plan_person_agenda_award_array) == 3
    assert plan_person_agenda_award_array[0][0] == exx.bob
    assert plan_person_agenda_award_array[1][0] == exx.yao
    assert plan_person_agenda_award_array[2][0] == exx.zia
    assert len(plan_person_agenda_award_array[0]) == 3
    assert len(plan_person_agenda_award_array[1]) == 3
    assert len(plan_person_agenda_award_array[2]) == 3
    assert plan_person_agenda_award_array[0][1] == bob_fund_agenda_take
    assert plan_person_agenda_award_array[0][2] == bob_fund_agenda_give
    assert plan_person_agenda_award_array[1][1] == yao_fund_agenda_take
    assert plan_person_agenda_award_array[1][2] == yao_fund_agenda_give


def test_get_plan_person_agenda_award_csv_ReturnsObj_ScenarioMultiplePersonUnit():
    # ESTABLISH
    yao_fund_agenda_give = 17
    yao_fund_agenda_take = 23
    bob_fund_agenda_give = 17
    ...
    # WHEN
    plan_person_agenda_award_csv_str = get_plan_person_agenda_award_csv(sue_plan)

    # THEN
    print(f"{plan_person_agenda_award_csv_str=}")
    print("")
...
"""
    ...
    assert plan_person_agenda_award_csv_str == example_csv_str


def test_get_plan_person_agenda_award_csv_ReturnsObj_cashout_True():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    sue_plan.add_personunit(exx.yao)
    sue_plan.add_personunit(exx.bob)
...
{exx.zia},0,0
"""
    assert empty_person_agenda_award == get_plan_person_agenda_award_csv(sue_plan)

    # WHEN
    plan_person_agenda_award_csv_str = get_plan_person_agenda_award_csv(
...
def test_get_person_mandate_ledger_ReturnsObj_Scenario0_MultiplePersonUnit():
    ...
    # WHEN
    plan_bud_net_dict = get_person_mandate_ledger(sue_plan)
...
def test_get_person_mandate_ledger_ReturnsObj_Scenario1_cashout_True():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    sue_plan.add_personunit(exx.yao, 13, 5)
    sue_plan.add_personunit(exx.bob, 5, 7)
    sue_plan.add_personunit(exx.xio, 2, 3)
    sue_plan.add_personunit(exx.zia, 0, 0)
    ...
    assert get_person_mandate_ledger(sue_plan) == pre_settle_person_mandate_ledger

    # WHEN
    sue_plan_settle_net_dict = get_person_mandate_ledger(sue_plan, cashout=True)

    # THEN
    assert sue_plan_settle_net_dict != pre_settle_person_mandate_ledger
...
def test_get_person_mandate_ledger_ReturnsObj_Scenario2_No_personunits():
    ...
    empty_person_mandate_ledger = {exx.sue: sue_plan.fund_pool}

    # WHEN / THEN
    assert get_person_mandate_ledger(sue_plan) == empty_person_mandate_ledger


def test_get_person_mandate_ledger_ReturnsObj_Scenario3_No_planunit():
    # ESTABLISH / WHEN / THEN
...
def test_get_person_agenda_net_ledger_ReturnsObj_ScenarioMultiplePersonUnit():
    ...
    # WHEN
    plan_bud_net_dict = get_person_agenda_net_ledger(sue_plan)
...
def test_get_person_agenda_net_ledger_ReturnsObj_cashout_True():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    sue_plan.add_personunit(exx.yao, 13, 5)
    sue_plan.add_personunit(exx.bob, 5, 7)
    sue_plan.add_personunit(exx.xio, 2, 3)
    sue_plan.add_personunit(exx.zia, 0, 0)
    assert get_person_agenda_net_ledger(sue_plan) == {}

    # WHEN
    sue_plan_settle_net_dict = get_person_agenda_net_ledger(sue_plan, cashout=True)
...
def test_get_credit_ledger_ReturnsObj_Scenario1_personunits_Exist():
    # ESTABLISH
    sue_plan = planunit_shop(exx.sue)
    bob_person_cred_lumen = 11
    yao_person_cred_lumen = 13
    xio_person_cred_lumen = 17
    sue_plan.add_personunit(exx.yao, yao_person_cred_lumen)
    sue_plan.add_personunit(exx.bob, bob_person_cred_lumen)
    sue_plan.add_personunit(exx.xio, xio_person_cred_lumen)
    ...
    expected_sue_credit_ledger = {
        exx.bob: bob_person_cred_lumen,
        exx.yao: yao_person_cred_lumen,
        exx.xio: xio_person_cred_lumen,
    }
