...
from src.ch07_plan_logic.plan_tool import (
    ...
    plan_partner_membership_get_obj,
    plan_partnerunit_get_obj,
)
from src.ref.keywords import Ch07Keywords as kw, ExampleStrs as exx


def test_plan_partnerunit_get_obj_ReturnsObj():
    # ESTABLISH
    ...
    # WHEN
    x_obj = plan_partnerunit_get_obj(sue_plan, jkeys)
    # THEN
    assert x_obj
    assert x_obj == sue_plan.get_partner(exx.yao)


def test_plan_partner_membership_get_obj_ReturnsObj():
    # ESTABLISH
    swim_str = ";swim"
    sue_plan = planunit_shop("Sue")
    jkeys = {kw.partner_name: exx.yao, "group_title": swim_str}
    sue_plan.add_partnerunit(exx.yao)
    sue_plan.get_partner(exx.yao).add_membership(swim_str)

    # WHEN
    x_obj = plan_partner_membership_get_obj(sue_plan, jkeys)
    # THEN
    assert x_obj
    assert x_obj == sue_plan.get_partner(exx.yao).get_membership(swim_str)
...
def test_plan_get_obj_ReturnsObj_plan_partnerunit_get_obj():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    jkeys = {kw.partner_name: exx.yao}
    sue_plan.add_partnerunit(exx.yao)
    ...
    # WHEN
    x_obj = plan_get_obj(kw.plan_partnerunit, sue_plan, jkeys)
    # THEN
    assert x_obj
    assert x_obj == sue_plan.get_partner(exx.yao)


def test_plan_get_obj_ReturnsObj_plan_partner_membership_get_obj():
    # ESTABLISH
    swim_str = ";swim"
    sue_plan = planunit_shop("Sue")
    jkeys = {kw.partner_name: exx.yao, "group_title": swim_str}
    ...
    # WHEN
    x_obj = plan_get_obj(kw.plan_partner_membership, sue_plan, jkeys)
    # THEN
    assert x_obj
    assert x_obj == sue_plan.get_partner(exx.yao).get_membership(swim_str)
