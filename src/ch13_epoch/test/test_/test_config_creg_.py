...
def test_PlanUnit_create_agenda_keg_CreatesAllPlanAttributes():
    ...
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    assert len(sue_plan.voices) == 0
    assert len(sue_plan.get_voiceunit_group_titles_dict()) == 0
    ...
    assert len(sue_plan.get_voiceunit_group_titles_dict()) == 0
    assert sue_plan.get_voiceunit_group_titles_dict().get(family_str) is None
