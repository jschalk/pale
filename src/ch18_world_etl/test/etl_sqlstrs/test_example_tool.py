...
def test_insert_prncase_special_h_agg_PopulatesTable_Scenario0():
    # ESTABLISH
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        create_sound_and_heard_tables(cursor)
        ...
        s1_reason_upper = 500
        s1_reason_lower = 600
        assert get_row_count(cursor, prncase_h_agg) == 0
...
def test_select_prncase_special_h_agg_PopulatesTable_Scenario0():
    # ESTABLISH
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        create_sound_and_heard_tables(cursor)
