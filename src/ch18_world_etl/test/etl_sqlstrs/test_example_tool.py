...
def test_insert_nabtime_h_agg_otx_inx_time_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    spark1 = 1
    ...
    otx_time = 5
    inx_time = 8
    assert get_row_count(cursor0, nabtime_h_agg_tablename) == 0

    # WHEN
    insert_nabtime_h_agg_otx_inx_time(
        cursor0, spark1, exx.sue, exx.a23, otx_time, inx_time
    )

    # THEN
    assert get_row_count(cursor0, nabtime_h_agg_tablename) == 1
...
def test_select_nabtime_h_agg_otx_inx_time_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    spark1 = 1
    ...
    otx_time = 5
    inx_time = 8
    assert not select_nabtime_h_agg_otx_inx_time(cursor0, spark1, exx.a23)
    ...
    gen_rows = select_nabtime_h_agg_otx_inx_time(cursor0, spark1, exx.a23)
    assert gen_rows == expected_rows


def test_insert_mmtunit_special_c400_number_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    spark1 = 1
    ...
    x_c400_number = 5
    assert get_row_count(cursor0, mmtunit_h_agg_tablename) == 0

    # WHEN
    insert_mmtunit_special_c400_number(cursor0, spark1, exx.sue, exx.a23, x_c400_number)

    # THEN
    assert get_row_count(cursor0, mmtunit_h_agg_tablename) == 1
    select_sqlstr = f"""SELECT 
  {kw.spark_num}
, {kw.face_name}
, {kw.moment_rope}
...
def test_insert_mmtoffi_special_offi_time_otx_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    spark1 = 1
    ...
    x_offi_time_otx = 5
    assert get_row_count(cursor0, mmtoffi_h_agg_tablename) == 0

    # WHEN
    insert_mmtoffi_special_offi_time_otx(
        cursor0, spark1, exx.sue, exx.a23, x_offi_time_otx
    )

    # THEN
    assert get_row_count(cursor0, mmtoffi_h_agg_tablename) == 1
    select_sqlstr = f"""SELECT 
  {kw.spark_num}
, {kw.face_name}
, {kw.moment_rope}
...
def test_select_mmtoffi_special_offi_time_inx_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    spark1 = 1
    ...
    mmtoffi_h_agg_tablename = prime_tbl(kw.moment_timeoffi, "h", "agg")
    x_offi_time_otx = 5
    assert not select_mmtoffi_special_offi_time_inx(cursor0, spark1, exx.a23)
    ...
    # THEN
    assert select_mmtoffi_special_offi_time_inx(cursor0, spark1, exx.a23)
    gen_rows = select_mmtoffi_special_offi_time_inx(cursor0, spark1, exx.a23)
...
def test_insert_prncase_special_h_agg_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    create_sound_and_heard_tables(cursor0)
    ...
    s1_reason_upper = 500
    s1_reason_lower = 600
    assert get_row_count(cursor0, prncase_h_agg) == 0
    ...
    # THEN
    assert get_row_count(cursor0, prncase_h_agg) == 1
    select_sqlstr = f"""SELECT 
  {kw.spark_num}
, {kw.person_name}
, {kw.plan_rope}
...
def test_select_prncase_special_h_agg_PopulatesTable_Scenario0(cursor0: Cursor):
    # ESTABLISH
    create_sound_and_heard_tables(cursor0)
    prncase_h_agg = prime_tbl(kw.person_plan_reason_caseunit, "h", "agg", "put")
    spark1 = 1
