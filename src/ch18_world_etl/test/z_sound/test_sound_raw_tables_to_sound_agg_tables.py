...
def test_set_sound_raw_tables_error_message_UpdatesTable_Scenario1_plan_raw_del():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        create_sound_and_heard_tables(cursor)
        plana_s_raw_del = create_prime_tablename(kw.plan_partnerunit, "s", "raw", "del")
...
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")
        plnptnr_s_put_raw_tblname = create_prime_tablename("PLNPTNR", "s", "raw", "put")
        insert_into_clause = f"""INSERT INTO {plnptnr_s_put_raw_tblname} (
  {kw.idea_number}
, {kw.spark_num}
, {kw.face_name}
, {kw.moment_rope}
...
"""
        ...
        plnptnr_s_put_agg_tblname = create_prime_tablename("PLNPTNR", "s", "agg", "put")
        assert get_row_count(cursor, trlrope_s_raw_tablename) == 7
        assert get_row_count(cursor, plnptnr_s_put_raw_tblname) == 6
        assert get_row_count(cursor, trlrope_s_agg_tablename) == 0
        assert get_row_count(cursor, plnptnr_s_put_agg_tblname) == 0
        ...
        assert get_row_count(cursor, plnptnr_s_put_agg_tblname) == 2
        ...
        select_agg_sqlstr = f"""SELECT * FROM {plnptnr_s_put_agg_tblname};"""
        cursor.execute(select_agg_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert len(rows) == 2
...
def test_insert_sound_raw_selects_into_sound_agg_tables_PopulatesValidTable_Scenario1_del_table():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        create_sound_and_heard_tables(cursor)
        plnptnr_s_del_raw_tblname = create_prime_tablename("PLNPTNR", "s", "raw", "del")
        insert_into_clause = f"""INSERT INTO {plnptnr_s_del_raw_tblname} (
  {kw.idea_number}
, {kw.spark_num}
, {kw.face_name}
, {kw.moment_rope}
...
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")
        plnptnr_s_del_agg_tblname = create_prime_tablename("PLNPTNR", "s", "agg", "del")
        assert get_row_count(cursor, plnptnr_s_del_raw_tblname) == 6
        assert get_row_count(cursor, plnptnr_s_del_agg_tblname) == 0

        # WHEN
        insert_sound_raw_selects_into_sound_agg_tables(cursor)

        # THEN
        assert get_row_count(cursor, plnptnr_s_del_agg_tblname) == 3

        select_agg_sqlstr = f"""SELECT * FROM {plnptnr_s_del_agg_tblname};"""
        cursor.execute(select_agg_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [
...
"""
        ...
        assert get_row_count(cursor, plnptnr_s_put_raw_tblname) == 7
        assert get_row_count(cursor, trlrope_s_agg_tablename) == 0
        assert get_row_count(cursor, plnptnr_s_put_agg_tblname) == 0
        ...
        assert get_row_count(cursor, plnptnr_s_put_agg_tblname) == 3
