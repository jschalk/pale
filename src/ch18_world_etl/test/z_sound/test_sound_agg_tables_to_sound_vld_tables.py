...
def test_get_insert_into_sound_vld_sqlstrs_ReturnsObj_PopulatesTable_Scenario0():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        planavoice_s_agg_put_tablename = prime_tbl(kw.plan_voiceunit, "s", "agg", "put")
        print(f"{get_table_columns(cursor, planavoice_s_agg_put_tablename)=}")
        insert_into_clause = f"""INSERT INTO {planavoice_s_agg_put_tablename} (
  {kw.spark_num}
, {kw.face_name}
, {kw.moment_label}
, {kw.plan_name}
...
"""
        ...
        assert get_row_count(cursor, planavoice_s_agg_put_tablename) == 4
        plnawar_h_vld_put_tablename = prime_tbl(kw.plan_voiceunit, "s", "vld", "put")
...
def test_etl_sound_agg_tables_to_sound_vld_tables_Scenario0_AddRowsToTable():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        create_sound_and_heard_tables(cursor)
        plnvoce_s_agg_put_tablename = prime_tbl(kw.plan_voiceunit, "s", "agg", "put")
        print(f"{get_table_columns(cursor, plnvoce_s_agg_put_tablename)=}")
        insert_into_clause = f"""INSERT INTO {plnvoce_s_agg_put_tablename} (
  {kw.spark_num}
, {kw.face_name}
, {kw.moment_label}
, {kw.plan_name}
...
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")
        assert get_row_count(cursor, plnvoce_s_agg_put_tablename) == 4
        plnvoce_h_vld_put_tablename = prime_tbl(kw.plan_voiceunit, "s", "vld", "put")
        assert get_row_count(cursor, plnvoce_h_vld_put_tablename) == 0

        # WHEN
        etl_sound_agg_tables_to_sound_vld_tables(cursor)

        # THEN
        assert get_row_count(cursor, plnvoce_h_vld_put_tablename) == 4
        select_sqlstr = f"""SELECT {kw.spark_num}
, {kw.face_name}
, {kw.moment_label}
, {kw.plan_name}
...
, {kw.voice_cred_lumen}
, {kw.voice_debt_lumen}
FROM {plnvoce_h_vld_put_tablename}
"""
        cursor.execute(select_sqlstr)
        rows = cursor.fetchall()
        print(rows)
...
"""
        ...
        # THEN
        assert get_row_count(cursor, plnvoce_h_vld_put_tablename) == 3
        select_sqlstr = f"""SELECT {kw.spark_num}
, {kw.face_name}
, {kw.moment_label}
, {kw.plan_name}
