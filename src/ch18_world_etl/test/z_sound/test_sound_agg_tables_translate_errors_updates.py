...
from src.ch18_world_etl.etl_sqlstr import (
    ...
    CREATE_PLNPTNR_SOUND_PUT_AGG_SQLSTR,
...
def test_create_knot_exists_in_name_error_update_sqlstr_ReturnsObj_PopulatesTable_Scenario0():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        cursor.execute(CREATE_PLNPTNR_SOUND_PUT_AGG_SQLSTR)
        plnptnr_dimen = kw.plan_partnerunit
        plnptnr_s_agg_put = create_prime_tablename(plnptnr_dimen, "s", "agg", "put")
        insert_plnptnr_sqlstr = f"""INSERT INTO {plnptnr_s_agg_put} (
  {kw.spark_num}, {kw.face_name}, {kw.moment_rope}, {kw.plan_name}, {kw.partner_name})
VALUES
  ({spark1}, '{exx.sue}', '{exx.a23}', '{exx.yao}', '{exx.yao}')
, ({spark1}, '{exx.sue}', '{exx.a23}', '{exx.yao}', '{bob_str}')
;
"""
        cursor.execute(insert_plnptnr_sqlstr)
...
"""
        ...
        error_count_sqlstr = f"SELECT COUNT(*) FROM {plnptnr_s_agg_put} WHERE {kw.error_message} IS NOT NULL"
        assert cursor.execute(error_count_sqlstr).fetchone()[0] == 0

        # WHEN
        sqlstr = create_knot_exists_in_name_error_update_sqlstr(
            plnptnr_s_agg_put, kw.partner_name
        )
        print(f"{sqlstr=}")
        ...
        select_core_raw_sqlstr = f"SELECT * FROM {plnptnr_s_agg_put}"
...
"""
        ...
        # THEN
        assert cursor.execute(error_count_sqlstr).fetchone()[0] == 1
        select_core_raw_sqlstr = f"SELECT * FROM {plnptnr_s_agg_put} ORDER BY {kw.moment_rope}, {kw.plan_name}, {kw.partner_name}"
        cursor.execute(select_core_raw_sqlstr)
        name_knot_str = f"Knot cannot exist in NameTerm column {kw.partner_name}"
