...
from src.ch18_world_etl.transformers import (
    ...
    insert_rose_core_agg_to_rose_core_vld_table,
    insert_rose_core_raw_to_rose_core_agg_table,
    insert_rose_sound_agg_into_rose_core_raw_table,
    insert_rose_sound_agg_tables_to_rose_sound_vld_table,
    populate_rose_core_vld_with_missing_face_names,
    update_inconsistency_rose_core_raw_table,
    update_rose_sound_agg_inconsist_errors,
    update_rose_sound_agg_knot_errors,
)
...
def test_create_insert_into_rose_core_raw_sqlstr_ReturnsObj_PopulatesTable_Scenario0():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        rose_rope_s_agg_tablename = create_prime_tablename(trlrope_dimen, "s", "agg")
        insert_into_clause = f"""INSERT INTO {rose_rope_s_agg_tablename} (
  {kw.spark_num}
, {kw.face_name}
, {kw.otx_rope}
, {kw.inx_rope}
...
"""
        ...
        assert get_row_count(cursor, rose_core_s_raw_tablename) == 0

        # WHEN
        sqlstr = create_insert_into_rose_core_raw_sqlstr(trlrope_dimen)
        print(f"{sqlstr=}")
        cursor.execute(sqlstr)

        # THEN
        assert get_row_count(cursor, rose_core_s_raw_tablename) == 3
        select_core_raw_sqlstr = f"SELECT * FROM {rose_core_s_raw_tablename}"
        cursor.execute(select_core_raw_sqlstr)
        assert cursor.fetchall() == [
            (rose_rope_s_agg_tablename, "Sue", None, None, None, None),
            (rose_rope_s_agg_tablename, "Sue", ":", ":", "Unknown", None),
            (rose_rope_s_agg_tablename, "Yao", ":", ":", "Unknown", None),
        ]
...
def test_insert_rose_sound_agg_into_rose_core_raw_table_PopulatesTable_Scenario0_IgnoresDimen_rose_epoch():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        rose_epoc_s_agg_tablename = create_prime_tablename(trlepoc_dimen, "s", "agg")
        insert_into_clause = f"""INSERT INTO {rose_epoc_s_agg_tablename} (
  {kw.spark_num}
, {kw.face_name}
, {kw.otx_epoch_length}
, {kw.inx_epoch_diff}
...
"""
        ...
        rose_core_s_raw_tablename = create_prime_tablename("trlcore", "s", "raw")
        assert get_row_count(cursor, rose_epoc_s_agg_tablename) == 3
        assert get_row_count(cursor, rose_core_s_raw_tablename) == 0
...
"""
        ...
        rose_name_s_agg_tablename = create_prime_tablename(trlname_dimen, "s", "agg")
        insert_into_clause = f"""INSERT INTO {rose_name_s_agg_tablename} (
  {kw.spark_num}
, {kw.face_name}
, {kw.otx_name}
, {kw.inx_name}
...
"""
        ...
        create_sound_and_heard_tables(cursor)
        rose_core_s_raw_tablename = create_prime_tablename("trlcore", "s", "raw")
        assert get_row_count(cursor, rose_rope_s_agg_tablename) == 3
        assert get_row_count(cursor, rose_name_s_agg_tablename) == 2
        assert get_row_count(cursor, rose_core_s_raw_tablename) == 0
        ...
        # THEN
        assert get_row_count(cursor, rose_core_s_raw_tablename) == 4
        select_core_raw_sqlstr = f"SELECT * FROM {rose_core_s_raw_tablename}"
        cursor.execute(select_core_raw_sqlstr)
        rows = cursor.fetchall()
        print(f"{rows=}")
        assert rows == [
            (rose_name_s_agg_tablename, "Bob", ":", ":", "Unknown", None),
            (rose_name_s_agg_tablename, "Sue", None, None, None, None),
            (rose_rope_s_agg_tablename, "Sue", None, None, None, None),
            (rose_rope_s_agg_tablename, "Yao", ":", ":", "Unknown", None),
        ]


def test_update_inconsistency_rose_core_raw_table_UpdatesTable_Scenario0():
    # ESTABLISH
    yao_inx = "Yaoito"
    bob_inx = "Bobito"
    rdx = ":"
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        rose_core_s_raw_tablename = create_prime_tablename(trlcore_dimen, "s", "raw")
        insert_into_clause = f"""INSERT INTO {rose_core_s_raw_tablename} (
  source_dimen
, {kw.face_name}
, {kw.otx_knot}
, {kw.inx_knot}
...
, {kw.error_message}
        ...
        values_clause = f"""
VALUES
  ('{rose_name_s_agg_tablename}', "{exx.bob}", "{rdx}", "{rdx}", "{ukx}", NULL)
, ('{rose_name_s_agg_tablename}', "{exx.sue}", NULL, NULL, '{rdx}', NULL)
, ('{rose_rope_s_agg_tablename}', "{exx.sue}", NULL, NULL, '{other_knot}', NULL)
, ('{rose_rope_s_agg_tablename}', "{exx.yao}", "{rdx}", "{rdx}", "{ukx}", NULL)
;
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")

        create_sound_and_heard_tables(cursor)
        select_error_count_sqlstr = f"SELECT COUNT(*) FROM {rose_core_s_raw_tablename} WHERE {kw.error_message} IS NOT NULL;"
        ...
        assert rows == [
            (rose_name_s_agg_tablename, "Bob", ":", ":", "Unknown", None),
            (rose_name_s_agg_tablename, "Sue", None, None, ":", error_data_str),
            (rose_rope_s_agg_tablename, "Sue", None, None, "/", error_data_str),
            (rose_rope_s_agg_tablename, "Yao", ":", ":", "Unknown", None),
        ]


def test_insert_rose_core_raw_to_rose_core_agg_table_PopulatesTable_Scenario0():
    # ESTABLISH
    yao_inx = "Yaoito"
    bob_inx = "Bobito"
    rdx = ":"
...
VALUES
  ('{rose_name_s_agg_tablename}', "{exx.bob}", "{rdx}", "{rdx}", "{ukx}", NULL)
, ('{rose_name_s_agg_tablename}', "{exx.sue}", NULL, NULL, '{rdx}', '{error_data_str}')
, ('{rose_rope_s_agg_tablename}', "{exx.sue}", NULL, NULL, '{other_knot}', '{error_data_str}')
, ('{rose_rope_s_agg_tablename}', "{exx.yao}", "{rdx}", "{rdx}", "{ukx}", NULL)
;
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")
        ...
        create_sound_and_heard_tables(cursor)
        rose_core_s_agg_tablename = create_prime_tablename(trlcore_dimen, "s", "agg")
        assert get_row_count(cursor, rose_core_s_agg_tablename) == 0

        # WHEN
        insert_rose_core_raw_to_rose_core_agg_table(cursor)

        # THEN
        assert get_row_count(cursor, rose_core_s_agg_tablename) == 2
        select_core_raw_sqlstr = f"SELECT * FROM {rose_core_s_agg_tablename}"
        cursor.execute(select_core_raw_sqlstr)
        rows = cursor.fetchall()
        print(f"{rows=}")
...
def test_insert_rose_core_agg_to_rose_core_vld_table_PopulatesTable_Scenario0():
    # ESTABLISH
    colon_knot = ":"
    slash_knot = "/"
    other_knot = "="
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        trlcore_dimen = kw.rose_core
        rose_core_s_agg_tablename = create_prime_tablename(trlcore_dimen, "s", "agg")
        insert_into_clause = f"""INSERT INTO {rose_core_s_agg_tablename} (
  {kw.face_name}
, {kw.otx_knot}
, {kw.inx_knot}
, {kw.unknown_str}
...
"""
        ...
        rose_core_s_vld_tablename = create_prime_tablename(trlcore_dimen, "s", "vld")
        assert get_row_count(cursor, rose_core_s_vld_tablename) == 0

        # WHEN
        insert_rose_core_agg_to_rose_core_vld_table(cursor)

        # THEN
        assert get_row_count(cursor, rose_core_s_vld_tablename) == 4
        select_core_raw_sqlstr = f"SELECT * FROM {rose_core_s_vld_tablename}"
        cursor.execute(select_core_raw_sqlstr)
        rows = cursor.fetchall()
        print(f"{rows=}")
        assert rows == [
...
def test_create_update_rose_sound_agg_inconsist_sqlstr_PopulatesTable_Scenario0():
    ...
    spark5 = 5
    spark7 = 7
    error_rose_str = "Inconsistent rose core data"
...
"""
        ...
        insert_into_clause = f"""INSERT INTO {rose_core_s_vld_tablename} (
  {kw.face_name}
, {kw.otx_knot}
, {kw.inx_knot}
, {kw.unknown_str}
...
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")
        select_error_count_sqlstr = f"SELECT COUNT(*) FROM {rose_rope_s_agg_tablename} WHERE {kw.error_message} IS NOT NULL;"
        assert cursor.execute(select_error_count_sqlstr).fetchone()[0] == 0

        # WHEN
        sqlstr = create_update_rose_sound_agg_inconsist_sqlstr(trlrope_dimen)
        print(f"{sqlstr=}")
        cursor.execute(sqlstr)

        # THEN
        assert cursor.execute(select_error_count_sqlstr).fetchone()[0] == 5
        select_core_raw_sqlstr = f"SELECT * FROM {rose_rope_s_agg_tablename}"
        cursor.execute(select_core_raw_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [
            (1, exx.sue, exx.yao, yao_inx, None, None, None, error_rose_str),
            (1, exx.sue, exx.bob, bob_inx, None, None, None, error_rose_str),
            (1, exx.sue, exx.bob, exx.bob, None, "/", None, error_rose_str),
            (2, exx.sue, exx.sue, exx.sue, ":", ":", "Unknown", error_rose_str),
            (5, exx.sue, exx.bob, bob_inx, ":", ":", "Unknown", error_rose_str),
            (7, exx.yao, exx.yao, yao_inx, ":", ":", "Unknown", None),
        ]


def test_update_rose_sound_agg_inconsist_errors_PopulatesTable_Scenario1():
    # ESTABLISH
    yao_inx = "Yaoito"
    bob_inx = "Bobito"
    rdx = ":"
...
"""
        ...
        cursor.execute(CREATE_TRLCORE_SOUND_VLD_SQLSTR)
        rose_core_s_vld_tablename = create_prime_tablename("trlcore", "s", "vld")
        insert_sqlstr = f"""INSERT INTO {rose_core_s_vld_tablename} (
  {kw.face_name}
, {kw.otx_knot}
, {kw.inx_knot}
, {kw.unknown_str}
...
"""
  ...
  FROM rose_label_s_agg label_agg
  JOIN rose_core_s_vld core_vld ON core_vld.face_name = label_agg.face_name
...
"""
  ...
  FROM rose_rope_s_agg rope_agg
  JOIN rose_core_s_vld core_vld ON core_vld.face_name = rope_agg.face_name
...
"""
  ...
  FROM rose_name_s_agg name_agg
  JOIN rose_core_s_vld core_vld ON core_vld.face_name = name_agg.face_name
...
"""
  ...
  FROM rose_title_s_agg title_agg
  JOIN rose_core_s_vld core_vld ON core_vld.face_name = title_agg.face_name
  WHERE NOT ((
...
def test_update_rose_sound_agg_knot_errors_UpdatesTables_Scenario0():
    # ESTABLISH
    casa_str = "Casa"
    rdx = ":"
    ukx = "Unknown"
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        cursor.execute(CREATE_TRLNAME_SOUND_AGG_SQLSTR)
        cursor.execute(CREATE_TRLTITL_SOUND_AGG_SQLSTR)
        trllabe_s_agg_tablename = create_prime_tablename(kw.rose_label, "s", "agg")
        trlrope_s_agg_tablename = create_prime_tablename(kw.rose_rope, "s", "agg")
        trlname_s_agg_tablename = create_prime_tablename(kw.rose_name, "s", "agg")
        trltitl_s_agg_tablename = create_prime_tablename(kw.rose_title, "s", "agg")
...
def test_create_insert_rose_sound_vld_table_sqlstr_ReturnsObj_PopulatesTable_Scenario0():
    # ESTABLISH
    yao_inx = "Yaoito"
    bob_inx = "Bobito"
    rdx = ":"
...
, {kw.error_message}
        ...
        values_clause = f"""
VALUES
  ({spark1}, '{exx.sue}', '{exx.yao}', '{yao_inx}', NULL, NULL, NULL, '{error_rose_str}')
, ({spark1}, '{exx.sue}', '{exx.bob}', '{bob_inx}', NULL, NULL, NULL, '{error_rose_str}')
, ({spark1}, '{exx.sue}', '{exx.bob}', '{exx.bob}', NULL, '{other_knot}', NULL, '{error_rose_str}')
, ({spark2}, '{exx.sue}', '{exx.sue}', '{exx.sue}', '{rdx}', '{rdx}', '{ukx}', '{error_rose_str}')
, ({spark5}, '{exx.sue}', '{exx.bob}', '{bob_inx}', '{rdx}', '{rdx}', '{ukx}', '{error_rose_str}')
...
"""
        cursor.execute(f"{insert_into_clause} {values_clause}")
        rose_rope_s_vld_tablename = create_prime_tablename("trlrope", "s", "vld")
        assert get_row_count(cursor, rose_rope_s_agg_tablename) == 8
        assert get_row_count(cursor, rose_rope_s_vld_tablename) == 0

        # WHEN
        sqlstr = create_insert_rose_sound_vld_table_sqlstr(trlrope_dimen)
        print(sqlstr)
        cursor.execute(sqlstr)

        # THEN
        assert get_row_count(cursor, rose_rope_s_vld_tablename) == 3
        select_rose_rope_s_vld_sqlstr = f"SELECT * FROM {rose_rope_s_vld_tablename}"
        cursor.execute(select_rose_rope_s_vld_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [
...
def test_insert_rose_sound_agg_tables_to_rose_sound_vld_table_PopulatesTable_Scenario0():
    # ESTABLISH
    yao_inx = "Yaoito"
    bob_inx = "Bobito"
    rdx = ":"
...
def test_etl_rose_sound_agg_tables_to_rose_sound_vld_tables_Scenario0_PopulatesTable():
    # ESTABLISH
    yao_inx = "Yaoito"
    bob_inx = "Bobito"
    rdx = ":"
...
"""
        ...
        rose_name_s_vld_tablename = create_prime_tablename("trlname", "s", "vld")
        assert get_row_count(cursor, rose_name_s_agg_tablename) == 10
        select_error_count_sqlstr = f"SELECT COUNT(*) FROM {rose_name_s_agg_tablename} WHERE {kw.error_message} IS NOT NULL;"
        assert cursor.execute(select_error_count_sqlstr).fetchone()[0] == 0
        assert get_row_count(cursor, rose_core_s_raw_tablename) == 6
        assert get_row_count(cursor, rose_core_s_agg_tablename) == 0
        assert get_row_count(cursor, rose_name_s_vld_tablename) == 0

        # WHEN
        etl_rose_sound_agg_tables_to_rose_sound_vld_tables(cursor)

        # THEN
        rose_name_s_agg_select = f"SELECT * FROM {rose_name_s_agg_tablename};"
        print(f"{cursor.execute(rose_name_s_agg_select).fetchall()=}\n")
        rose_core_s_raw_select = f"SELECT * FROM {rose_core_s_raw_tablename};"
        print(f"{cursor.execute(rose_core_s_raw_select).fetchall()=}\n")
        rose_core_s_agg_select = f"SELECT * FROM {rose_core_s_agg_tablename};"
        print(f"{cursor.execute(rose_core_s_agg_select).fetchall()=}\n")
        assert cursor.execute(select_error_count_sqlstr).fetchone()[0] == 5
        assert get_row_count(cursor, rose_core_s_agg_tablename) == 2
        assert get_row_count(cursor, rose_name_s_vld_tablename) == 3
        select_rose_name_s_vld_sqlstr = f"SELECT * FROM {rose_name_s_vld_tablename}"
        cursor.execute(select_rose_name_s_vld_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [
            (spark1, exx.yao, exx.yao, exx.yao),
...
def test_etl_rose_sound_agg_tables_to_rose_sound_vld_tables_Scenario1_UpdatesErrors():
    # ESTABLISH
    casa_str = "Casa"
    rdx = ":"
    ukx = "Unknown"
...
def test_populate_rose_core_vld_with_missing_face_names_Scenario0_Populates1MissingRoseCoreRow():
    # ESTABLISH
    casa_str = "Casa"
    rdx = ":"
    ukx = "Unknown"
...
def test_populate_rose_core_vld_with_missing_face_names_Scenario1_PopulatesSomeMissingRoseCoreRows():
    # ESTABLISH
    rdx = ":"
    ukx = "Unknown"
    spark1 = 1
...
def test_etl_rose_sound_agg_tables_to_rose_sound_vld_tables_Scenario2_Populates1MissingRoseCoreRow():
    # ESTABLISH
    casa_str = "Casa"
    rdx = ":"
    ukx = "Unknown"
...
def test_etl_rose_sound_agg_tables_to_rose_sound_vld_tables_Scenario3_PopulatesSomeMissingRoseCoreRows():
    # ESTABLISH
    rdx = ":"
    ukx = "Unknown"
    spark1 = 1
