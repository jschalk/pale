...
"""
        ...
        assert not db_table_exists(cursor, plnptnr_s_put_raw_tblname)
        ...
        assert get_row_count(cursor, plnptnr_s_put_raw_tblname) == 2
        b117 = "br00117"
        b045 = "br00045"
        ...
        select_agg_sqlstr = f"""SELECT * FROM {plnptnr_s_put_raw_tblname};"""
        cursor.execute(select_agg_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert len(rows) == 2
