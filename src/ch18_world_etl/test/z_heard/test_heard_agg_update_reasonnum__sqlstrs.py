from src.ch18_world_etl.etl_sqlstr import (
    ...
    get_update_prncase_context_keg_sqlstr,
    get_update_prncase_inx_epoch_diff_sqlstr,
    get_update_prncase_range_sqlstr,
    get_update_prnfact_context_keg_sqlstr,
...
def test_get_update_prncase_context_keg_sqlstr_ReturnsObj():
    # ESTABLISH
    prncase_tablename = prime_tbl(kw.person_keg_reason_caseunit, "h", "agg", "put")
    prnkegg_tablename = prime_tbl(kw.person_kegunit, "h", "agg", "put")

    # WHEN
    update_sqlstr = get_update_prncase_context_keg_sqlstr()
...
UPDATE {prncase_tablename}
SET 
  context_keg_close = spark_prnkegg.{kw.close}
, context_keg_denom = spark_prnkegg.{kw.denom}
, context_keg_morph = spark_prnkegg.{kw.morph}
FROM spark_prnkegg
...
def test_get_update_prnfact_context_keg_sqlstr_ReturnsObj():
    # ESTABLISH
    prnfact_tablename = prime_tbl(kw.prnfact, "h", "agg", "put")
    prnkegg_tablename = prime_tbl(kw.person_kegunit, "h", "agg", "put")

    # WHEN
    update_sqlstr = get_update_prnfact_context_keg_sqlstr()
