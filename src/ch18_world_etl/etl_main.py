...
def etl_heard_vld_tables_to_moment_jsons(cursor: sqlite3_Cursor, moment_mstr_dir: str):
    ...
    for moment_rope_set in cursor.fetchall():
        moment_rope = moment_rope_set[0]
        moment_dict = get_moment_dict_from_heard_tables(cursor, moment_rope)
...
"""
    ...
    for moment_rope in get_level1_dirs(moments_dir):
        ote1_csv_path = create_moment_ote1_csv_path(moment_mstr_dir, moment_rope)
...
def etl_moment_ote1_agg_csvs_to_jsons(moment_mstr_dir: str):
    ...
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        csv_path = create_moment_ote1_csv_path(moment_mstr_dir, moment_rope)
        ...
        json_path = create_moment_ote1_json_path(moment_mstr_dir, moment_rope)
...
def etl_create_moment_cell_trees(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        create_moment_plans_cell_trees(moment_mstr_dir, moment_rope)


def etl_set_cell_trees_found_facts(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        set_cell_trees_found_facts(moment_mstr_dir, moment_rope)


def etl_set_cell_trees_decrees(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        set_cell_trees_decrees(moment_mstr_dir, moment_rope)


def etl_set_cell_tree_cell_mandates(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        set_cell_tree_cell_mandates(moment_mstr_dir, moment_rope)


def etl_create_bud_mandate_ledgers(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        create_bud_mandate_ledgers(moment_mstr_dir, moment_rope)
...
def etl_spark_plan_csvs_to_lesson_json(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        moment_path = create_path(moments_dir, moment_rope)
        ...
        for plan_name in get_level1_dirs(plans_path):
            ...
            for spark_num in get_level1_dirs(sparks_path):
                ...
                spark_all_lesson_path = create_spark_all_lesson_path(
                    moment_mstr_dir, moment_rope, plan_name, spark_num
                )
                save_json(
        ...
        for plan_name in get_level1_dirs(plans_dir):
            ...
            for spark_num in get_level1_dirs(sparks_dir):
                prev_plan = _get_prev_spark_num_planunit(
                    moment_mstr_dir, moment_rope, plan_name, prev_spark_num
                )
...
def _get_prev_spark_num_planunit(
    ...
    if prev_spark_num is None:
        return planunit_shop(plan_name, moment_rope)
...
def etl_spark_inherited_planunits_to_moment_gut(moment_mstr_dir: str):
    moments_dir = create_path(moment_mstr_dir, "moments")
    for moment_rope in get_level1_dirs(moments_dir):
        plan_sparks = collect_plan_spark_dir_sets(moment_mstr_dir, moment_rope)
        ...
        for plan_name, max_spark_num in plans_max_spark_num_dict.items():
            max_planspark_path = create_planspark_path(
                moment_mstr_dir, moment_rope, plan_name, max_spark_num
            )
...
def etl_moment_job_jsons_to_job_tables(cursor: sqlite3_Cursor, moment_mstr_dir: str):
    ...
    for moment_rope in get_level1_dirs(moments_dir):
        ...
        plans_dir = create_path(moment_path, "plans")
        for plan_name in get_level1_dirs(plans_dir):
            job_obj = open_job_file(moment_mstr_dir, moment_rope, plan_name)
...
def insert_tranunit_persons_net(cursor: sqlite3_Cursor, tranbook: TranBook):
    ...
    cursor.executemany(
        f"INSERT INTO moment_person_nets (moment_rope, plan_name, plan_net_amount) VALUES ('{tranbook.moment_rope}', ?, ?)",
        persons_net_array,
    )
