...
@dataclass
class RoseUnit:
    """Per face object that roses any translatable str.
    ...
    unknown_str: str = None  # roseunit
    otx_knot: KnotTerm = None  # roseunit
    inx_knot: KnotTerm = None  # roseunit
    ...
    def _check_attr_match(self, attr: str, mapcore):
        ...
        if self_attr != unit_attr:
            exception_str = f"set_mapcore Error: RoseUnit {attr} is '{self_attr}', MapCore is '{unit_attr}'."
    ...
    def to_dict(self) -> dict:
        ...
        x_namemap = _get_rid_of_rose_core_keys(self.namemap.to_dict())
        x_titlemap = _get_rid_of_rose_core_keys(self.titlemap.to_dict())
        x_labelmap = _get_rid_of_rose_core_keys(self.labelmap.to_dict())
        x_ropemap = _get_rid_of_rose_core_keys(self.ropemap.to_dict())
...
def get_roseunit_from_dict(x_dict: dict) -> RoseUnit:
    ...
    namemap_dict = _add_rose_core_keys(
        namemap_dict,
        x_spark_num,
        x_face_name,
        x_otx_knot,
    ...
    titlemap_dict = _add_rose_core_keys(
        titlemap_dict,
        x_spark_num,
        x_face_name,
        x_otx_knot,
    ...
    labelmap_dict = _add_rose_core_keys(
        labelmap_dict,
        x_spark_num,
        x_face_name,
        x_otx_knot,
    ...
    ropemap_dict = _add_rose_core_keys(
        ropemap_dict,
        x_spark_num,
        x_face_name,
        x_otx_knot,
...
def _get_rid_of_rose_core_keys(map_dict: dict) -> dict:
...
class RoseCoreAttrConflictException(Exception):
    pass


def inherit_roseunit(older: RoseUnit, newer: RoseUnit) -> RoseUnit:
    if (
        ...
        raise RoseCoreAttrConflictException("Core attrs in conflict")
    if older.spark_num >= newer.spark_num:
        raise RoseCoreAttrConflictException("older roseunit is not older")
