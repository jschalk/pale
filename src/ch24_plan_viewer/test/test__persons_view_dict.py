...
from src.ch24_plan_viewer.plan_viewer_tool import add_small_dot, get_persons_view_dict
from src.ref.keywords import Ch24Keywords as kw, ExampleStrs as exx


def test_get_persons_view_dict_ReturnsObj_Scenario0_Empty():
    # ESTABLISH
    sue_believer = planunit_shop(exx.sue)
    sue_believer.cashout()

    # WHEN
    persons_view_dict = get_persons_view_dict(sue_believer)

    # THEN
    assert persons_view_dict == {}
...
def test_get_persons_view_dict_ReturnsObj_Scenario1_persons():
    # ESTABLISH
    sue_believer = planunit_shop(exx.sue)
    yao_cred_lumen = 110
    yao_debt_lumen = 130
    bob_cred_lumen = 230
    bob_debt_lumen = 290
    sue_believer.add_personunit(exx.yao, yao_cred_lumen, yao_debt_lumen)
    sue_believer.add_personunit(exx.bob, bob_cred_lumen, bob_debt_lumen)
    sue_believer.cashout()
    ...
    # THEN
    assert set(persons_view_dict.keys()) == {exx.yao, exx.bob}
    yao_person_dict = persons_view_dict.get(exx.yao)
    person_cred_lumen_readable_key = add_readable(kw.person_cred_lumen)
    person_debt_lumen_readable_key = add_readable(kw.person_debt_lumen)
    ...
    irrational_person_debt_lumen_readable_key = add_readable(
        kw.irrational_person_debt_lumen
    )
    inallocable_person_debt_lumen_readable_key = add_readable(
    ...
    assert set(yao_person_dict.keys()) == {
        kw.person_name,
        ...
        person_cred_lumen_readable_key,
        person_debt_lumen_readable_key,
        memberships_readable_key,
        credor_pool_readable_key,
        debtor_pool_readable_key,
        irrational_person_debt_lumen_readable_key,
        inallocable_person_debt_lumen_readable_key,
    ...
    ypu = sue_believer.get_person(exx.yao)
    yp_dict = yao_person_dict
    assert ypu.person_name == yp_dict.get(kw.person_name)
    assert ypu.person_cred_lumen == yp_dict.get(kw.person_cred_lumen)
    assert ypu.person_debt_lumen == yp_dict.get(kw.person_debt_lumen)
    assert ypu.credor_pool == yp_dict.get(kw.credor_pool)
    assert ypu.debtor_pool == yp_dict.get(kw.debtor_pool)
    assert ypu.irrational_person_debt_lumen == yp_dict.get(
        kw.irrational_person_debt_lumen
    )
    assert ypu.inallocable_person_debt_lumen == yp_dict.get(
        kw.inallocable_person_debt_lumen
    )
    ...
    expected_person_cred_lumen_readable = f"person_cred_lumen: {ypu.person_cred_lumen}"
    expected_person_debt_lumen_readable = f"person_debt_lumen: {ypu.person_debt_lumen}"
    ...
    expected_irrational_person_debt_lumen_readable = (
        f"irrational_person_debt_lumen: {ypu.irrational_person_debt_lumen}"
    )
    expected_inallocable_person_debt_lumen_readable = (
        f"inallocable_person_debt_lumen: {ypu.inallocable_person_debt_lumen}"
    )
    ...
    assert (
        yp_dict.get(person_cred_lumen_readable_key)
        == expected_person_cred_lumen_readable
    )
    assert (
        yp_dict.get(person_debt_lumen_readable_key)
        == expected_person_debt_lumen_readable
    )
    ...
    assert (
        yp_dict.get(irrational_person_debt_lumen_readable_key)
        == expected_irrational_person_debt_lumen_readable
    )
    assert (
        yp_dict.get(inallocable_person_debt_lumen_readable_key)
        == expected_inallocable_person_debt_lumen_readable
    )
...
def test_get_persons_view_dict_ReturnsObj_Scenario2_memberships():
    # ESTABLISH
    sue_believer = planunit_shop(exx.sue)
    sue_believer.add_personunit(exx.yao)
    swim_str = ";swimmers"
    yao_swim_cred_lumen = 311
    yao_swim_debt_lumen = 313
    yao_personunit = sue_believer.get_person(exx.yao)
    yao_personunit.add_membership(swim_str, yao_swim_cred_lumen, yao_swim_debt_lumen)
    sue_believer.cashout()
    ...
    # THEN
    assert set(persons_view_dict.keys()) == {exx.yao}
    yao_person_dict = persons_view_dict.get(exx.yao)
    assert kw.memberships in set(yao_person_dict.keys())
    yao_memberships_dict = yao_person_dict.get(kw.memberships)
    ...
    yao_swim_mu = yao_personunit.get_membership(swim_str)
    ...
    assert yao_swim_dict.get(kw.person_name) == yao_swim_mu.person_name
    ...
    # yao_personunit = sue_believer.get_person(exx.yao)
    # bob_personunit = sue_believer.get_person(exx.bob)
    # bob_personunit.add_membership(swim_str, bob_swim_cred_lumen, bob_swim_debt_lumen)
    # yao_personunit.add_membership(swim_str, yao_swim_cred_lumen, yao_swim_debt_lumen)
    # yao_personunit.add_membership(clea_str, cleaners_cred_lumen, cleaners_debt_lumen)
    # sue_believer.get_person(exx.yao).add_membership()
