...
from src.ch24_plan_viewer.test._util.ch24_examples import ExampleValuesRef as wx
...
def test_create_gcalendar_events_list_ReturnsObj_Scenario0_Empty():
    ...
    # WHEN
    sue_gcal_events = create_gcalendar_events_list(sue_plan, apr7)

    # THEN
    assert sue_gcal_events == []


def test_PlanUnit_cashout_SetsAttr_ScenarioX_SingleBranch_fund_ratio():
    # ESTABLISH
    sue_plan = planunit_shop(wx.sue, wx.a23)
    sue_plan.add_keg(wx.mop_rope, pledge=True, star=1)
    mop_keg = sue_plan.get_keg_obj(wx.mop_rope)
    assert not sue_plan.kegroot.fund_onset
    assert not sue_plan.kegroot.fund_cease
    assert not sue_plan.kegroot.fund_ratio
    ...
    # THEN
    assert sue_plan.kegroot.fund_onset == 0
    assert sue_plan.kegroot.fund_cease == 1000000000.0
    assert sue_plan.kegroot.fund_ratio == 1.0
    assert mop_keg.fund_onset == 0
    assert mop_keg.fund_cease == sue_plan.fund_pool
...
def test_create_gcalendar_events_list_ReturnsObj_Scenario2_3AllDayPledge():
    # ESTABLISH
    sue_plan = planunit_shop(wx.sue, wx.a23)
    sue_plan.add_keg(wx.mop_rope, pledge=True, star=2)
    sue_plan.add_keg(wx.sweep_rope, pledge=True, star=1)
    sue_plan.add_keg(wx.scrub_rope, pledge=True, star=1)
    apr7 = datetime(2010, 5, 7, 9)
    print(f"{apr7=}")
...
#     add_epoch_kegunit(sue_plan, default_epoch_config)
#     sue_plan.edit_reason(wx.mop_rope, reason_context=, reason_case=)
#     apr7 = datetime(2010, 5, 7, 9)
#     print(f"{apr7=}")
