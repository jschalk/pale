...
from src.ch24_plan_viewer.plan_viewer_tool import add_small_dot, get_partners_view_dict
...
def test_get_partners_view_dict_ReturnsObj_Scenario1_partners():
    ...
    # THEN
    assert set(partners_view_dict.keys()) == {exx.yao, exx.bob}
    yao_partner_dict = partners_view_dict.get(exx.yao)
    partner_cred_lumen_readable_key = add_readable(kw.partner_cred_lumen)
    partner_debt_lumen_readable_key = add_readable(kw.partner_debt_lumen)
    ...
    irrational_partner_debt_lumen_readable_key = add_readable(
        kw.irrational_partner_debt_lumen
    )
    inallocable_partner_debt_lumen_readable_key = add_readable(
    ...
    assert set(yao_partner_dict.keys()) == {
        ...
        partner_cred_lumen_readable_key,
        partner_debt_lumen_readable_key,
        memberships_readable_key,
        credor_pool_readable_key,
        debtor_pool_readable_key,
        irrational_partner_debt_lumen_readable_key,
        inallocable_partner_debt_lumen_readable_key,
    ...
    ypu = sue_believer.get_partner(exx.yao)
    yp_dict = yao_partner_dict
    assert ypu.partner_name == yp_dict.get(kw.partner_name)
    assert ypu.partner_cred_lumen == yp_dict.get(kw.partner_cred_lumen)
    assert ypu.partner_debt_lumen == yp_dict.get(kw.partner_debt_lumen)
    assert ypu.credor_pool == yp_dict.get(kw.credor_pool)
    assert ypu.debtor_pool == yp_dict.get(kw.debtor_pool)
    assert ypu.irrational_partner_debt_lumen == yp_dict.get(
        kw.irrational_partner_debt_lumen
    )
    assert ypu.inallocable_partner_debt_lumen == yp_dict.get(
        kw.inallocable_partner_debt_lumen
    )
    ...
    expected_partner_cred_lumen_readable = (
        f"partner_cred_lumen: {ypu.partner_cred_lumen}"
    )
    expected_partner_debt_lumen_readable = (
        f"partner_debt_lumen: {ypu.partner_debt_lumen}"
    )
    ...
    expected_irrational_partner_debt_lumen_readable = (
        f"irrational_partner_debt_lumen: {ypu.irrational_partner_debt_lumen}"
    )
    expected_inallocable_partner_debt_lumen_readable = (
        f"inallocable_partner_debt_lumen: {ypu.inallocable_partner_debt_lumen}"
    )
    ...
    assert (
        yp_dict.get(partner_cred_lumen_readable_key)
        == expected_partner_cred_lumen_readable
    )
    assert (
        yp_dict.get(partner_debt_lumen_readable_key)
        == expected_partner_debt_lumen_readable
    )
    ...
    assert (
        yp_dict.get(irrational_partner_debt_lumen_readable_key)
        == expected_irrational_partner_debt_lumen_readable
    )
    assert (
        yp_dict.get(inallocable_partner_debt_lumen_readable_key)
        == expected_inallocable_partner_debt_lumen_readable
    )
...
def test_get_partners_view_dict_ReturnsObj_Scenario2_memberships():
    ...
    # THEN
    assert set(partners_view_dict.keys()) == {exx.yao}
    yao_partner_dict = partners_view_dict.get(exx.yao)
    ...
    # yao_partnerunit = sue_believer.get_partner(exx.yao)
    # bob_partnerunit = sue_believer.get_partner(exx.bob)
