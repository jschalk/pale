...
def plan_objs_asdict(
    obj: Any, current_plan: PlanUnit = None, current_reason: ReasonUnit = None
) -> dict:
    ...
    if dataclasses.is_dataclass(obj):
        ...
        for field in dataclasses.fields(obj):
            ...
            result[field.name] = plan_objs_asdict(value, current_plan, current_reason)
        if isinstance(obj, KegUnit):
            ...
            obj_readable_str = get_fact_state_readable_str(obj, None, current_plan)
            result["readable"] = add_small_dot(obj_readable_str)
        ...
        b = current_plan
        r = current_reason
        return [plan_objs_asdict(v, b, r) for v in obj]
    elif isinstance(obj, dict):
        b = current_plan
        r = current_reason
        return {k: plan_objs_asdict(v, b, r) for k, v in obj.items()}
    else:
        return obj
...
def get_groups_view_dict(plan: PlanUnit) -> dict[str,]:
    ...
    for group in plan.groupunits.values():
