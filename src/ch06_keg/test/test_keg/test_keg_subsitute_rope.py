...
def test_KegUnit_find_replace_rope_Modifies_parent_rope():
    # ESTABLISH Keg with _parent_rope that will be different
    old_casa_str = "casa1"
    old_first_label = "YY"
    ...
    x_keg = kegunit_shop(tulips_str, parent_rope=old_bloomers_rope)
    assert create_rope(x_keg.parent_rope) == old_bloomers_rope
    assert create_rope(x_keg.parent_rope, x_keg.keg_label) == old_tulips_rope

    # WHEN
    new_casa = "casa2"
    new_casa_rope = create_rope(old_first_label, new_casa)
    x_keg.find_replace_rope(old_rope=old_casa_rope, new_rope=new_casa_rope)

    # THEN
    new_bloomers_rope = create_rope(new_casa_rope, bloomers_str)
    new_tulips_rope = create_rope(new_bloomers_rope, tulips_str)
    assert create_rope(x_keg.parent_rope) == new_bloomers_rope
    assert create_rope(x_keg.parent_rope, x_keg.keg_label) == new_tulips_rope


def test_KegUnit_find_replace_rope_Modifies_reasonunits():
    # ESTABLISH Keg with reason that will be different
    casa_str = "casa1"
    old_first_label = "YY"
    ...
    x_keg = kegunit_shop(tulips_str, reasonunits=reasons_x)
    # check asserts
    assert x_keg.reasonunits.get(old_water_rope) is not None
    old_water_rain_reason = x_keg.reasonunits[old_water_rope]
    ...
    assert x_keg.reasonunits.get(new_water_rope) is None
    x_keg.find_replace_rope(old_rope=old_water_rope, new_rope=new_water_rope)

    # THEN
    assert x_keg.reasonunits.get(old_water_rope) is None
    assert x_keg.reasonunits.get(new_water_rope) is not None
    new_water_rain_reason = x_keg.reasonunits[new_water_rope]
    ...
    reason_obj = x_keg.reasonunits.get(new_water_rope)
...
def test_KegUnit_find_replace_rope_Modifies_factunits():
    # ESTABLISH Keg with factunit that will be different
    tulips_str = "tulips"
    old_water_str = "water"
    old_first_label = "YY"
    old_water_rope = create_rope(old_first_label, old_water_str)
    ...
    x_keg = kegunit_shop(tulips_str, factunits=factunits_x)
    assert x_keg.factunits[old_water_rope] is not None
    old_water_rain_factunit = x_keg.factunits[old_water_rope]
    ...
    new_water_str = "h2o"
    new_water_rope = create_rope(old_first_label, new_water_str)
    assert x_keg.factunits.get(new_water_rope) is None
    x_keg.find_replace_rope(old_rope=old_water_rope, new_rope=new_water_rope)

    # THEN
    assert x_keg.factunits.get(old_water_rope) is None
    assert x_keg.factunits.get(new_water_rope) is not None
    new_water_rain_factunit = x_keg.factunits[new_water_rope]
    assert new_water_rain_factunit.fact_context == new_water_rope
    new_rain_rope = create_rope(new_water_rope, rain_str)
    assert new_water_rain_factunit.fact_state == new_rain_rope

    print(f"{len(x_keg.factunits)=}")
    x_factunit = x_keg.factunits.get(new_water_rope)
