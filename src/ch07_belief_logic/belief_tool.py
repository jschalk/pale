...
def belief_kegunit_exists(x_belief: BeliefUnit, jkeys: dict[str, any]) -> bool:
    x_rope = jkeys.get("keg_rope")
    return False if x_belief is None else bool(x_belief.keg_exists(x_rope))


def belief_keg_awardunit_exists(x_belief: BeliefUnit, jkeys: dict[str, any]) -> bool:
    x_awardee_title = jkeys.get("awardee_title")
    x_rope = jkeys.get("keg_rope")
    return bool(
        belief_kegunit_exists(x_belief, jkeys)
        and x_belief.get_keg_obj(x_rope).awardunit_exists(x_awardee_title)
    )


def belief_keg_reasonunit_exists(x_belief: BeliefUnit, jkeys: dict[str, any]) -> bool:
    x_rope = jkeys.get("keg_rope")
    x_reason_context = jkeys.get("reason_context")
    return bool(
        belief_kegunit_exists(x_belief, jkeys)
        and x_belief.get_keg_obj(x_rope).reasonunit_exists(x_reason_context)
    )


def belief_keg_reason_caseunit_exists(
    ...
    return bool(
        belief_keg_reasonunit_exists(x_belief, jkeys)
        and x_belief.get_keg_obj(x_rope)
        .get_reasonunit(x_reason_context)
        .case_exists(x_reason_state)
    )


def belief_keg_partyunit_exists(x_belief: BeliefUnit, jkeys: dict[str, any]) -> bool:
    x_party_title = jkeys.get("party_title")
    x_rope = jkeys.get("keg_rope")
    return bool(
        belief_kegunit_exists(x_belief, jkeys)
        and x_belief.get_keg_obj(x_rope).laborunit.partyunit_exists(x_party_title)
    )


def belief_keg_healerunit_exists(x_belief: BeliefUnit, jkeys: dict[str, any]) -> bool:
    x_healer_name = jkeys.get("healer_name")
    x_rope = jkeys.get("keg_rope")
    return bool(
        belief_kegunit_exists(x_belief, jkeys)
        and x_belief.get_keg_obj(x_rope).healerunit.healer_name_exists(x_healer_name)
    )


def belief_keg_factunit_exists(x_belief: BeliefUnit, jkeys: dict[str, any]) -> bool:
    x_rope = jkeys.get("keg_rope")
    x_fact_context = jkeys.get("fact_context")
    return bool(
        belief_kegunit_exists(x_belief, jkeys)
        and x_belief.get_keg_obj(x_rope).factunit_exists(x_fact_context)
    )
...
def belief_attr_exists(
    ...
    elif x_dimen == "belief_keg_awardunit":
        return belief_keg_awardunit_exists(x_belief, jkeys)
    elif x_dimen == "belief_keg_factunit":
        return belief_keg_factunit_exists(x_belief, jkeys)
    elif x_dimen == "belief_keg_healerunit":
        return belief_keg_healerunit_exists(x_belief, jkeys)
    elif x_dimen == "belief_keg_reason_caseunit":
        return belief_keg_reason_caseunit_exists(x_belief, jkeys)
    elif x_dimen == "belief_keg_reasonunit":
        return belief_keg_reasonunit_exists(x_belief, jkeys)
    elif x_dimen == "belief_keg_partyunit":
        return belief_keg_partyunit_exists(x_belief, jkeys)
    elif x_dimen == "belief_kegunit":
        return belief_kegunit_exists(x_belief, jkeys)
...
def belief_kegunit_get_obj(x_belief: BeliefUnit, jkeys: dict[str, any]) -> KegUnit:
    x_rope = jkeys.get("keg_rope")
    return x_belief.get_keg_obj(x_rope)


def belief_keg_awardunit_get_obj(
    x_belief: BeliefUnit, jkeys: dict[str, any]
) -> AwardUnit:
    x_rope = jkeys.get("keg_rope")
    x_awardee_title = jkeys.get("awardee_title")
    return x_belief.get_keg_obj(x_rope).get_awardunit(x_awardee_title)


def belief_keg_reasonunit_get_obj(
    x_belief: BeliefUnit, jkeys: dict[str, any]
) -> ReasonUnit:
    x_rope = jkeys.get("keg_rope")
    x_reason_context = jkeys.get("reason_context")
    return x_belief.get_keg_obj(x_rope).get_reasonunit(x_reason_context)


def belief_keg_reason_caseunit_get_obj(
    x_belief: BeliefUnit, jkeys: dict[str, any]
) -> CaseUnit:
    ...
    x_reason_state = jkeys.get("reason_state")
    return (
        x_belief.get_keg_obj(x_rope)
        .get_reasonunit(x_reason_context)
        .get_case(x_reason_state)
    )


def belief_keg_factunit_get_obj(
    x_belief: BeliefUnit, jkeys: dict[str, any]
) -> FactUnit:
    x_rope = jkeys.get("keg_rope")
    x_fact_context = jkeys.get("fact_context")
    return x_belief.get_keg_obj(x_rope).factunits.get(x_fact_context)
...
def belief_get_obj(x_dimen: str, x_belief: BeliefUnit, jkeys: dict[str, any]) -> any:
    ...
    x_dimens = {
        ...
        "belief_kegunit": belief_kegunit_get_obj,
        "belief_keg_awardunit": belief_keg_awardunit_get_obj,
        "belief_keg_reasonunit": belief_keg_reasonunit_get_obj,
        "belief_keg_reason_caseunit": belief_keg_reason_caseunit_get_obj,
        "belief_keg_factunit": belief_keg_factunit_get_obj,
    }
...
def get_belief_root_facts_dict(
    ...
    return x_belief.get_kegroot_factunits_dict()


def set_factunits_to_belief(x_belief: BeliefUnit, x_facts_dict: dict[RopeTerm, dict]):
    """Sets dict of FactUnits to BeliefUnit kegroot"""
    ...
    not_missing_fact_reason_contexts = set(x_belief.get_kegroot_factunits_dict().keys())
...
def clear_factunits_from_belief(x_belief: BeliefUnit):
    """Deletes all BeliefUnit kegroot FactUnits"""
    for fact_reason_context in get_belief_root_facts_dict(x_belief).keys():
        x_belief.del_fact(fact_reason_context)


def belief_keg_reason_caseunit_set_obj(belief: BeliefUnit, args: dict[str,]):
    """Wrapper for method that edit beliefunit keg nodes reasonunits.
...
def get_belief_unique_short_ropes(belief: BeliefUnit) -> dict[RopeTerm, RopeTerm]:
    ...
    return get_unique_short_ropes(set(belief.get_keg_dict().keys()), belief.knot)
...
def add_frame_to_beliefunit(
    ...
    for x_keg in x_belief.get_keg_dict().values():
        for x_reason in x_keg.reasonunits.values():
            if is_sub_rope(x_reason.reason_context, required_context_subrope):
                reason_context_keg = x_belief.get_keg_obj(x_reason.reason_context)
        ...
        for x_fact in x_keg.factunits.values():
            if is_sub_rope(x_fact.fact_context, required_context_subrope):
                fact_context_keg = x_belief.get_keg_obj(x_fact.fact_context)
