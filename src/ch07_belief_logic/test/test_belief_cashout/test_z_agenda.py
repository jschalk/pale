...
def test_BeliefUnit_get_agenda_dict_ReturnsObj_WithTwoKegs():
    # ESTABLISH
...
def test_BeliefUnit_get_agenda_dict_ReturnsAgendaWithOnlyCorrectKegs():
    # ESTABLISH
...
def test_BeliefUnit_get_agenda_dict_WithLargeBelief_fund():
    ...
    print(f"{agenda_dict.keys()=} {x_belief.make_l1_rope(exx.casa)=}")
    print(f"{agenda_dict.get(x_belief.make_l1_rope(exx.casa)).keg_label=}")
    assert agenda_dict.get(x_belief.make_l1_rope(exx.casa)).fund_ratio


def test_BeliefUnit_get_agenda_dict_WithNo7amKegExample():
    # ESTABLISH
    ...
    cat_agenda_keg = agenda_dict.get(x_belief.make_l1_rope(cat_str))
    assert cat_agenda_keg.keg_label != clean_str


def test_BeliefUnit_get_agenda_dict_With7amKegExample():
    # ESTABLISH
    ...
    # THEN
    print(x_belief.kegroot.factunits[x24hr_rope])
    print(x_belief.get_keg_obj(clean_rope).reasonunits)
    print(x_belief.get_keg_obj(clean_rope).keg_active)
    agenda_dict = x_belief.get_agenda_dict()
...
def test_beliefunit_v001_AgendaExists():
    ...
    assert yao_belief
    # for keg_kid in yao_belief.kegroot.kids.values():
...
def test_BeliefUnit_get_agenda_dict_BeliefUnitHasCorrectAttributes_beliefunit_v001():
    ...
    # WHEN
    keg_pledge_list = yao_belief.get_agenda_dict()
...
def test_BeliefUnit_get_agenda_dict_BeliefUnitCanCleanOn_reason_context_beliefunit_v001_with_large_agenda():
    ...
    # for agenda_keg in yao_belief.get_agenda_dict():
    #     print(
...
def test_BeliefUnit_set_agenda_task_as_complete_SetsAttr_Range():
    ...
    zia_belief.set_l1_keg(kegunit_shop(run_str, pledge=True))
    zia_belief.set_keg_obj(kegunit_shop(jour_str, begin=0, close=500), ziet_rope)
    zia_belief.edit_keg_attr(
        run_rope,
    ...
    run_reasonunits = zia_belief.kegroot.kids[run_str].reasonunits[jour_rope]
    ...
    assert len(zia_belief.get_keg_dict()) == 4
...
def test_BeliefUnit_set_agenda_task_as_complete_SetsAttr_Division():
    ...
    run_keg = zia_belief.get_keg_obj(run_rope)
...
def test_get_beliefunit_from_dict_LoadsPledgeFromJSON():
    ...
    # THEN
    assert len(yao_belief.get_keg_dict()) == 252
    print(f"{len(yao_belief.get_keg_dict())=}")
    ...
    veg_keg = yao_belief.get_keg_obj(veg_rope)
    assert not veg_keg.keg_active
    assert veg_keg.pledge

    # keg_list = yao_belief.get_keg_dict()
    # pledge_true_count = 0
    # for keg in keg_list:
...
def test_BeliefUnit_set_fact_Isue116Resolved_SetstaskAsTrue():
    ...
    pledge_keg_list = yao_belief.get_agenda_dict()
    ...
    evening_keg = yao_belief._keg_dict.get(evening_rope)
    # for keg_x in yao_belief.get_agenda_dict():
    #     # if keg_x.task != True:
...
def test_BeliefUnit_agenda_IsSetByLaborUnit_1VoiceGroup():
    ...
    casa_rope = yao_belief.make_l1_rope(exx.casa)
    yao_belief.set_l1_keg(kegunit_shop(exx.casa, pledge=True))
    ...
    # WHEN
    yao_belief.edit_keg_attr(casa_rope, laborunit=laborunit_sue)
    ...
    # WHEN
    yao_belief.edit_keg_attr(casa_rope, laborunit=laborunit_yao)
...
def test_BeliefUnit_get_agenda_dict_IsSetByLaborUnit_2VoiceGroup():
    ...
    # WHEN
    yao_belief.edit_keg_attr(casa_rope, laborunit=run_laborunit)
...
def test_BeliefUnit_get_all_pledges_ReturnsObj():
    ...
    zia_belief.set_keg_obj(kegunit_shop(couch_str), casa_rope)
    zia_belief.set_keg_obj(kegunit_shop(exx.clean, pledge=True), casa_rope)
    zia_belief.set_keg_obj(kegunit_shop(sweep_str, pledge=True), clean_rope)
    sweep_keg = zia_belief.get_keg_obj(sweep_rope)
    ...
    # THEN
    assert all_pledges_dict.get(sweep_rope) == zia_belief.get_keg_obj(sweep_rope)
    assert all_pledges_dict.get(clean_rope) == zia_belief.get_keg_obj(clean_rope)
