...
def test_TreeMetrics_Exists():
    ...
    assert x_tree_metrics.all_keg_uids_are_unique is None
...
def test_treemetrics_shop_ReturnsObj():
    ...
    assert x_tree_metrics.all_keg_uids_are_unique
...
def test_BeliefUnit_set_keg_dict_Scenario0():
    # ESTABLISH
    ...
    assert not root_keg.gogo_calc
    assert not root_keg.stop_calc
    assert yao_belief._keg_dict == {root_keg.get_keg_rope(): root_keg}
    assert yao_belief.reason_contexts == set()


def test_BeliefUnit_set_keg_dict_Scenario1():
    # ESTABLISH
    yao_belief = beliefunit_shop("Yao")
    ziet0_begin = 7
    ziet0_close = 31
...
def test_BeliefUnit_set_keg_dict_Clears_gogo_calc_stop_calc():
    # ESTABLISH
...
def test_BeliefUnit_set_keg_dict_Sets_reason_contexts():
    # ESTABLISH
    ...
    sue_belief.add_keg(polis_rope)
    sue_belief.add_keg(nation_rope)
...
def test_BeliefUnit_set_keg_CreatesKegUnitsUsedBy_reasonunits():
    # ESTABLISH
    sue_belief = get_beliefunit_with_4_levels()
    ...
    assert sue_belief.keg_exists(buildings_rope) is False

    # WHEN
    sue_belief.set_keg_obj(clean_cuisine_keg, cleaning_rope, create_missing_kegs=True)

    # THEN
    assert sue_belief.keg_exists(buildings_rope)
...
def test_get_sorted_keg_list_ReturnsObj_Scenario0_DefaultOrder_keg_keg_rope():
    ...
    # WHEN
    x_sorted_keg_list = get_sorted_keg_list(sue_belief._keg_dict)

    # THEN
    assert x_sorted_keg_list is not None
    assert len(x_sorted_keg_list) == 17
    assert x_sorted_keg_list[0] == sue_belief.kegroot
    assert x_sorted_keg_list[1] == sue_belief.get_keg_obj(casa_rope)
    assert x_sorted_keg_list[11] == sue_belief.get_keg_obj(mon_rope)
