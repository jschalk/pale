...
def test_BeliefUnit_get_relevant_ropes_ReturnsSimpleReasonUnitreason_context():
    ...
    sue_belief.set_keg_obj(floor_keg, parent_rope=casa_rope)

    unim_str = "unimportant"
    unim_rope = sue_belief.make_l1_rope(unim_str)
    unim_keg = kegunit_shop(unim_str)
    sue_belief.set_keg_obj(unim_keg, parent_rope=sue_belief.kegroot.get_keg_rope())
    ...
    sue_belief.set_keg_obj(situation_keg, parent_rope=casa_rope)
...
def test_BeliefUnit_get_relevant_ropes_ReturnsReasonUnitreason_contextAndDescendents():
    ...
    root_rope = x_belief.kegroot.get_keg_rope()
...
def test_BeliefUnit_get_relevant_ropes_ReturnSimple():
    ...
    yao_belief.set_l1_keg(min_range_keg)
    ...
    yao_belief.set_keg_obj(min_jours_keg, parent_rope=min_range_x_rope)

    # WHEN
    print(f"{yao_belief._keg_dict.keys()}")
    ...
    # min_jours_keg = yao_belief.get_keg_obj(min_jours_rope)


def test_BeliefUnit_get_inheritor_keg_list_ReturnsObj_Scenario0():
    # ESTABLISH
    ...
    yao_beliefunit.set_keg_obj(kegunit_shop(exx.wk, begin=0, close=10800), tech_rope)
    mon_str = "Mon"
    mon_rope = yao_beliefunit.make_rope(wk_rope, mon_str)
    yao_beliefunit.set_keg_obj(kegunit_shop(mon_str), wk_rope)
    yao_beliefunit.cashout()

    # WHEN
    x_inheritor_keg_list = yao_beliefunit.get_inheritor_keg_list(wk_rope, mon_rope)

    # # THEN
    assert len(x_inheritor_keg_list) == 2
    wk_keg = yao_beliefunit.get_keg_obj(wk_rope)
    mon_keg = yao_beliefunit.get_keg_obj(mon_rope)
