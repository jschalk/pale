...
def test_get_belief_config_dict_ReturnsObj_CheckArgDataTypesCorrect():
    ...
    assert g_class_type(cfig, blffact, jv, kw.fact_upper) == kw.CotoNum
    assert g_sqlitetype(cfig, blffact, jv, kw.fact_upper) == "REAL"
    assert g_popcashout(cfig, blffact, jv, kw.fact_upper) == False

    assert g_class_type(cfig, blffact, jv, kw.fact_lower) == kw.CotoNum
    ...
    assert g_class_type(cfig, blfcase, jv, kw.reason_upper) == kw.CotoNum
    assert g_sqlitetype(cfig, blfcase, jv, kw.reason_upper) == "REAL"
    assert g_popcashout(cfig, blfcase, jv, kw.reason_upper) == False

    assert g_class_type(cfig, blfcase, jv, kw.reason_lower) == kw.CotoNum
...
def test_get_belief_calc_args_type_dict_ReturnsObj():
    ...
    assert belief_calc_args_type_dict.get(kw.fact_upper) == kw.CotoNum
    assert belief_calc_args_type_dict.get(kw.fact_lower) == kw.CotoNum
    ...
    assert belief_calc_args_type_dict.get(kw.reason_upper) == kw.CotoNum
    assert belief_calc_args_type_dict.get(kw.reason_lower) == kw.CotoNum
