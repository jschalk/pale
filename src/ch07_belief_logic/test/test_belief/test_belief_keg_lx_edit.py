...
def test_BeliefUnit_edit_keg_label_FailsWhenKegDoesNotExist():
    # ESTABLISH
    yao_belief = beliefunit_shop("Yao")

    casa_rope = yao_belief.make_l1_rope(exx.casa)
    yao_belief.set_l1_keg(kegunit_shop(exx.casa))
    yao_belief.set_keg_obj(kegunit_shop(exx.swim), parent_rope=casa_rope)

    # WHEN / THEN
    no_keg_rope = yao_belief.make_l1_rope("bees")
    with pytest_raises(Exception) as excinfo:
        yao_belief.edit_keg_label(old_rope=no_keg_rope, new_keg_label="birds")
...
def test_BeliefUnit_find_replace_rope_Modifies_kids_Scenario1():
    ...
    yao_belief.set_l1_keg(kegunit_shop(casa_old_str))
    yao_belief.set_keg_obj(kegunit_shop(bloomers_str), parent_rope=old_casa_rope)
    yao_belief.set_keg_obj(kegunit_shop(tulips_str), parent_rope=old_bloomers_rope)
    yao_belief.set_keg_obj(kegunit_shop(red_str), parent_rope=old_tulips_rope)
    r_keg_tulips = yao_belief.get_keg_obj(old_tulips_rope)
    r_keg_bloomers = yao_belief.get_keg_obj(old_bloomers_rope)
    ...
    yao_belief.edit_keg_label(old_rope=old_casa_rope, new_keg_label=new_casa_str)

    # THEN
    assert yao_belief.kegroot.kids.get(new_casa_str) is not None
    assert yao_belief.kegroot.kids.get(casa_old_str) is None
...
def test_belief_edit_keg_label_Modifies_factunits():
    ...
    yao_belief.set_l1_keg(kegunit_shop(exx.casa))
    yao_belief.set_keg_obj(kegunit_shop(tulips_str), parent_rope=bloomers_rope)
    yao_belief.set_keg_obj(kegunit_shop(rain_str), parent_rope=old_water_rope)
    yao_belief.add_fact(fact_context=old_water_rope, fact_state=old_rain_rope)

    keg_x = yao_belief.get_keg_obj(tulips_rope)
    assert yao_belief.kegroot.factunits[old_water_rope] is not None
    old_water_rain_factunit = yao_belief.kegroot.factunits[old_water_rope]
    ...
    yao_belief.set_l1_keg(kegunit_shop(new_water_str))
    assert yao_belief.kegroot.factunits.get(new_water_rope) is None
    yao_belief.edit_keg_label(old_rope=old_water_rope, new_keg_label=new_water_str)

    # THEN
    assert yao_belief.kegroot.factunits.get(old_water_rope) is None
    assert yao_belief.kegroot.factunits.get(new_water_rope) is not None
    new_water_rain_factunit = yao_belief.kegroot.factunits[new_water_rope]
    ...
    assert yao_belief.kegroot.factunits.get(new_water_rope)
    x_factunit = yao_belief.kegroot.factunits.get(new_water_rope)
    # for factunit_key, x_factunit in yao_belief.kegroot.factunits.items():
...
def test_belief_edit_keg_label_ModifiesKegReasonUnitsScenario1():
    ...
    casa_keg = sue_belief.get_keg_obj(sue_belief.make_l1_rope("casa"))
    ...
    wed_keg = sue_belief.get_keg_obj(old_sem_jour_rope)
...
def test_belief_set_belief_name_ModifiesBoth():
    ...
    # mid_keg_label1 = "Yao"
    # sue_belief.edit_keg_label(old_rope=old_keg_label, new_keg_label=mid_keg_label1)
    # assert sue_belief.belief_name == old_keg_label
    # assert sue_belief.kegroot.keg_label == mid_keg_label1
...
def test_belief_edit_keg_label_RaisesErrorIfknotIsInLabel():
    # ESTABLISH
    sue_belief = get_beliefunit_with_4_levels_and_2reasons_2facts()
    old_sem_jour_str = "sem_jours"
    old_sem_jour_rope = sue_belief.make_l1_rope(old_sem_jour_str)
    ...
    assert (
        str(excinfo.value)
        == f"Cannot modify '{old_sem_jour_rope}' because new_keg_label {new_sem_jour_str} contains knot {sue_belief.knot}"
    )
