...
from src.ch17_idea.idea_db_tool import (
    get_dataframe_roseable_columns,
    rose_all_columns_dataframe,
    rose_single_column_dataframe,
)
from src.ref.keywords import Ch17Keywords as kw, ExampleStrs as exx


def test_get_dataframe_roseable_columns_ReturnsObj():
    # ESTABLISH / WHEN / THEN
    x_dt = DataFrame()
    assert get_dataframe_roseable_columns(x_dt) == set()
    x_dt = DataFrame(columns=[kw.voice_name])
    assert get_dataframe_roseable_columns(x_dt) == {kw.voice_name}
    x_dt = DataFrame(columns=[kw.voice_name, kw.voice_cred_lumen])
    assert get_dataframe_roseable_columns(x_dt) == {kw.voice_name}
    x_dt = DataFrame(columns=[kw.reason_context, kw.voice_name, kw.voice_cred_lumen])
    assert get_dataframe_roseable_columns(x_dt) == {
        kw.voice_name,
        kw.reason_context,
    }
    x_dt = DataFrame(columns=["calc_swim", kw.voice_name, kw.voice_cred_lumen])
    assert get_dataframe_roseable_columns(x_dt) == {kw.voice_name}


def test_rose_single_column_dataframe_ReturnsObj_Scenario0_VoiceName_EmptyDataFrame():
    # ESTABLISH
    voice_name_mapunit = namemap_shop()
    empty_dt = DataFrame(columns=[kw.voice_name])

    # WHEN
    gen_dt = rose_single_column_dataframe(empty_dt, voice_name_mapunit, kw.voice_name)

    # THEN
    pandas_assert_frame_equal(gen_dt, empty_dt)


def test_rose_single_column_dataframe_SetsParameterAttrs_Scenario0_VoiceName_5rows():
    # ESTABLISH
    xio_otx = "Xio"
    sue_otx = "Sue"
    bob_otx = "Bob"
    ...
    # WHEN
    rose_single_column_dataframe(otx_dt, voice_name_mapunit, kw.voice_name)

    # THEN
    assert otx_dt.iloc[0][kw.voice_name] == zia_otx
...
def test_rose_single_column_dataframe_SetsParameterAttrs_Scenario1_VoiceName_5rowsMultipleColumns():
...
def test_rose_all_columns_dataframe_SetsParameterAttrs_Scenario0_VoiceName():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    xio_otx = "Xio"
    sue_otx = "Sue"
    ...
    # WHEN
    rose_all_columns_dataframe(otx_dt, None)
...
def test_rose_all_columns_dataframe_SetsParameterAttrs_Scenario1_VoiceName():
    # sourcery skip: extract-duplicate-method
    # ESTABLISH
    xio_otx = "Xio"
    sue_otx = "Sue"
    ...
    sue_inx = "Suita"
    bob_inx = "Bobita"
    yao_roseunit = roseunit_shop(exx.yao)
    yao_roseunit.set_otx2inx(kw.NameTerm, xio_otx, xio_inx)
    yao_roseunit.set_otx2inx(kw.NameTerm, sue_otx, sue_inx)
    yao_roseunit.set_otx2inx(kw.NameTerm, bob_otx, bob_inx)
    otx_dt = DataFrame(columns=[kw.moment_label, kw.voice_name, kw.voice_cred_lumen])
    otx_dt.loc[0] = ["YY", zia_otx, 12]
    otx_dt.loc[1] = ["YY", sue_otx, 12]
    otx_dt.loc[2] = ["YY", bob_otx, 12]
    ...
    # WHEN
    rose_all_columns_dataframe(otx_dt, yao_roseunit)
...
def test_rose_all_columns_dataframe_SetsParameterAttrs_Scenario2_RodeUnit_get_casa_maison_roseunit_set_by_otx2inx():
    # ESTABLISH
    otx_amy45_str = "amy45"
    inx_amy87_str = "amy87"
    ...
    yao_roseunit = get_casa_maison_roseunit_set_by_otx2inx()
...
def test_rose_all_columns_dataframe_SetsParameterAttrs_Scenario3_RodeUnit_get_casa_maison_roseunit_set_by_label():
    # ESTABLISH
    otx_amy45_str = "amy45"
    inx_amy87_str = "amy87"
    otx_amy45_rope = to_rope(otx_amy45_str)
    ...
    sweep_otx_rope = create_rope(clean_otx_rope, sweep_str)
    sweep_inx_rope = create_rope(clean_inx_rope, sweep_str)
    yao_roseunit = get_casa_maison_roseunit_set_by_label()
    # print(f"{yao_roseunit=}")
    otx_dt = get_casa_maison_rope_otx_dt()
    old_otx_dt = copy_deepcopy(otx_dt)
    assert otx_dt.iloc[0][kw.reason_context] == otx_amy45_rope
    assert otx_dt.iloc[1][kw.reason_context] == casa_otx_rope
