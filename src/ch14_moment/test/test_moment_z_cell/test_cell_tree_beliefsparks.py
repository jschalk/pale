...
from src.ch11_bud._ref.ch11_path import create_beliefspark_path, create_cell_json_path
from src.ch11_bud.bud_filehandler import (
    cellunit_add_json_file,
    save_arbitrary_beliefspark,
)
from src.ch14_moment.moment_cell import load_cells_beliefspark
...
def test_load_cells_beliefspark_SetsFiles_Scenario0_NoFacts(
    ...
    spark300 = 300
    time5 = 5
    save_arbitrary_beliefspark(moment_mstr_dir, exx.a23, exx.bob, spark300)
    bob3_beliefspark_path = create_beliefspark_path(
        moment_mstr_dir, exx.a23, exx.bob, spark300
    )
    print(f"{bob3_beliefspark_path=}")
    cellunit_add_json_file(moment_mstr_dir, exx.a23, exx.bob, time5, spark300, [])
    bob5_cell_path = create_cell_json_path(moment_mstr_dir, exx.a23, exx.bob, time5)
    assert open_json(bob5_cell_path).get(kw.beliefspark_facts) == {}

    # WHEN
    load_cells_beliefspark(moment_mstr_dir, exx.a23)
...
def test_load_cells_beliefspark_SetsFiles_Scenario1_WithFacts(
    ...
    save_arbitrary_beliefspark(
        moment_mstr_dir, exx.a23, exx.bob, spark300, facts=x_facts
    )
    bob3_beliefspark_path = create_beliefspark_path(
        moment_mstr_dir, exx.a23, exx.bob, spark300
    )
...
def test_load_cells_beliefspark_SetsFiles_Scenario2_WithFacts_NotAtRoot(
    ...
    das = [exx.yao, exx.bob]
    cellunit_add_json_file(moment_mstr_dir, exx.a23, exx.bob, time5, spark300, das)
    bob5_cell_path = create_cell_json_path(
        moment_mstr_dir, exx.a23, exx.bob, time5, das
    )
    assert open_json(bob5_cell_path).get(kw.beliefspark_facts) == {}
