...
def test_create_bud_mandate_ledgers_Scenaro0_BudEmpty(temp_dir_setup):
    ...
    epochtime9 = 9
    bob9_bud_mandate_path = bud_mandate_path(mstr_dir, a23_str, exx.bob, epochtime9)
...
def test_create_bud_mandate_ledgers_Scenaro1_BudExists(temp_dir_setup):
    ...
    a23_json_path = create_moment_json_path(mstr_dir, a23_str)
    save_json(a23_json_path, None, amy23_moment.to_dict())
    bob37_cell_mandate_path = cell_mandate_path(mstr_dir, a23_str, exx.bob, tp37)
    bob_mandate = 777
...
def test_create_bud_mandate_ledgers_Scenaro2_Mutliple_cell_voice_mandate_ledgers(
    ...
    a23_json_path = create_moment_json_path(mstr_dir, a23_str)
    save_json(a23_json_path, None, amy23_moment.to_dict())
    b37_cell_mandate = cell_mandate_path(mstr_dir, a23_str, exx.bob, tp37)
    b37_sue_cell_path = cell_mandate_path(mstr_dir, a23_str, exx.bob, tp37, [exx.sue])
    b37_yao_cell_path = cell_mandate_path(mstr_dir, a23_str, exx.bob, tp37, [exx.yao])
    yz_anc = [exx.yao, exx.zia]
    b37_yao_zia_cell_path = cell_mandate_path(mstr_dir, a23_str, exx.bob, tp37, yz_anc)
