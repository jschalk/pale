...
def test_generate_ingest_list_ReturnsList_v1():
    ...
    zia_personunit.set_l1_keg(kegunit_shop(exx.clean, pledge=True))
    zia_debtor_pool = 78
    zia_resepect_bit = 2
    ...
    ingested_list = generate_ingest_list(
        keg_list=list(zia_personunit.get_agenda_dict().values()),
    ...
    clean_rope = zia_personunit.make_l1_rope(exx.clean)
    clean_kegunit = zia_personunit.get_keg_obj(clean_rope)
...
def test_generate_ingest_list_ReturnsList_v2():
    ...
    zia_personunit = personunit_shop(exx.zia)
    zia_personunit.set_l1_keg(kegunit_shop(exx.clean, pledge=True))
    zia_personunit.set_l1_keg(kegunit_shop(exx.cuisine, pledge=True))
    zia_debtor_pool = 32
    zia_resepect_bit = 2
    ...
    clean_kegunit = zia_personunit.get_keg_obj(clean_rope)
    cuisine_kegunit = zia_personunit.get_keg_obj(cuisine_rope)
...
def test_generate_ingest_list_ReturnsList_v3():
    ...
    zia_personunit = personunit_shop(exx.zia)
    zia_personunit.set_l1_keg(kegunit_shop(exx.clean, pledge=True))
    zia_personunit.set_l1_keg(kegunit_shop(exx.cuisine, star=3, pledge=True))
    zia_debtor_pool = 32
    zia_resepect_bit = 2
    assert len(zia_personunit.get_agenda_dict()) == 2
...
def test_generate_ingest_list_ReturnsList_v4():
    ...
    zia_personunit = personunit_shop(exx.zia)
    zia_personunit.set_l1_keg(kegunit_shop(exx.clean, pledge=True))
    zia_personunit.set_l1_keg(kegunit_shop(exx.cuisine, star=2, pledge=True))
    zia_debtor_pool = 32
    zia_resepect_bit = 2
    assert len(zia_personunit.get_agenda_dict()) == 2
