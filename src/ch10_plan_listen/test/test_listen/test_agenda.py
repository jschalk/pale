...
def test_listen_to_speaker_agenda_RaisesErrorIfPoolIsNotSet():
    ...
    # THEN
    assertion_fail_str = f"listener '{exx.yao}' plan is assumed to have {zia_planunit.plan_name} partnerunit."
...
def test_listen_to_speaker_agenda_ReturnsEqualPlan():
    # ESTABLISH
    yao_planunit = planunit_shop(exx.yao)
    yao_planunit.add_partnerunit(exx.zia)
    yao_planunit.set_partner_respect(100)
    zia_planunit = planunit_shop(exx.zia)
...
def test_listen_to_speaker_agenda_ReturnsSingletaskPlan():
    ...
    before_yao_planunit.add_partnerunit(exx.zia)
    yao_partner_partner_debt_lumen = 77
    before_yao_planunit.set_partner_respect(yao_partner_partner_debt_lumen)
    zia_clean_kegunit = kegunit_shop(exx.clean, pledge=True)
    zia_clean_kegunit.laborunit.add_party(exx.yao)
    zia_planunit = planunit_shop(exx.zia)
    zia_planunit.add_partnerunit(exx.yao)
...
def test_listen_to_speaker_agenda_ReturnsLevel2taskPlan():
    ...
    before_yao_planunit.set_partner_respect(yao_partner_debt_lumen)
    zia_planunit = planunit_shop(exx.zia)
    zia_planunit.add_partnerunit(exx.yao)
    zia_clean_kegunit = kegunit_shop(exx.clean, pledge=True)
    zia_clean_kegunit.laborunit.add_party(exx.yao)
