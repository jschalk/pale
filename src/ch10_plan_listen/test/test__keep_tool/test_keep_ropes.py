...
from src.ch10_plan_listen._ref.ch10_path import create_keep_duty_path
from src.ch10_plan_listen.keep_tool import get_keep_ropes, save_all_gut_dutys
...
def test_get_keep_ropes_RaisesErrorWhen_keeps_justified_IsFalse(
    ...
    sue_gut_plan.set_keg_obj(kegunit_shop(dallas_str), texas_rope)
...
def test_get_keep_ropes_RaisesErrorWhen_keeps_buildable_IsFalse(
    ...
    sue_gut_plan.edit_keg_attr(texas_rope, healerunit=healerunit_shop({exx.sue}))
    sue_gut_plan.cashout()
    assert sue_gut_plan.keeps_justified
    assert sue_gut_plan.keeps_buildable is False
    save_gut_file(env_dir(), sue_gut_plan)
...
def test_get_keep_ropes_ReturnsObj(temp_dir_setup, graphics_bool):
    ...
    sue_gut_plan.cashout()
    display_kegtree(sue_gut_plan, mode="Keep", graphics_bool=graphics_bool)
    save_gut_file(env_dir(), sue_gut_plan)
