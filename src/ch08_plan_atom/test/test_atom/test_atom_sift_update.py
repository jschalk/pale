...
def test_sift_atom_ReturnsObj_PlanAtom_UPDATE_plan_partnerunit():
    # ESTABLISH
...
def test_sift_atom_ReturnsObj_PlanAtom_UPDATE_plan_partner_membership():
    # ESTABLISH
    zia_run_group_debt_lumen = 76
    sue_plan = planunit_shop("Sue")
    sue_plan.add_partnerunit(exx.zia)
    sue_plan.get_partner(exx.zia).add_membership(exx.run)

    zia_atom = planatom_shop(kw.plan_partner_membership, kw.INSERT)
    zia_atom.set_arg(kw.partner_name, exx.zia)
