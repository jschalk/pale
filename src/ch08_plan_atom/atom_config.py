...
def atom_config_path() -> str:
    "Returns Path: ch08_plan_atom/atom_config.json"
    src_dir = create_path(os_getcwd(), "src")
    chapter_dir = create_path(src_dir, "ch08_plan_atom")
...
def get_normalized_plan_table_build() -> dict[str : dict[str, any]]:
    normal_tables_dict = {}
    atom_config = get_atom_config_dict()
...
def is_plan_dimen(dimen_str: str) -> bool:
    return dimen_str in get_plan_dimens()
...
def get_dimen_from_dict(x_row_dict: dict) -> str:
    x_get_plan_dimens = get_plan_dimens()
    for x_columnname in x_row_dict:
        for x_dimen in x_get_plan_dimens:
